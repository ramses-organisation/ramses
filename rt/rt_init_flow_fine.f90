...
SUBROUTINE rt_region_condinit(x, uu, dx, nn)
    ...
    do k = 1, rt_nregion
        ...
        if (rt_n_region(k) <= 0.0) rt_n_region(k) = smallnp
        group_ind = iGroups(rt_reg_group(k))
        if (rt_reg_group(k) <= 0 .or. rt_reg_group(k) > nGroups) cycle
        ! For "square" regions only:
        if (rt_region_type(k) == 'square') then
        ...
        if (rt_region_type(k) == 'point') then
            ! Volume element
            vol = dx_cgs ** ndim
