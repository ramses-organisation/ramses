...
:::success
**Topics**: Octree, grids versus cells, linked lists, iterating through grids and cells, finding neighbors
:::
...
The grids making up the computational domain are stored in a linked list using an ordering recipe chosen by the user. For more on ordering, see [Chapter "Domain decomposition", section 1.1](https://codimd.math.cnrs.fr/9S84MTEkQWik6Qxcbmdjaw#11-Hilbert-curve).
...
```
(More on communicators in the Chapter on MPI communication, [section 3.1](https://codimd.math.cnrs.fr/9S84MTEkQWik6Qxcbmdjaw#31-Communicator-variables)).
This data structure contain a list of the index of each grid, as found by iterating over the linked grid list (`head`, `next`).

The array `active` holds a communicator for each AMR level of the current MPI domain with ID `myid`. Each communicator the contain a list of the index of each grid, as found by iterating over the linked grid list using`headl` and `next`.
The array `boundary` is similar, but instead keeps track of the different physical boundary regions for each level. Its contents is gathered by starting at `headb`.


:::info
**Exercise:** Write code to fill `active(ilevel)` of the current MPI domain by iterating through the corresponding linked list.
:::spoiler **Solution**
```fortran=
...
:::info
**Exercise (part 2):** Write (pseudo-)code to execute each of the steps found in part 1 of the exercise.
:::

:::info
**Exercise (part 3):** Take a look at the different routines that are implemented in the file *nbor_utils.f90*. Which routine serves which purpose? Which routine could be used to get the neighboring cells in 1D? Compare your (pseudo-)code to what is implemented in *nbor_utils.f90*
:::spoiler **Solution**
...
:::info
**Exercise (part 4):** What would change in the case of 2D? Make a distinction between retrieving the 4 direct neighbors and including also the diagonal neighbors.
:::
...
:::info
**Exercise:** Figure out the arrays `lll` and `mmm` in `amr_constants.f90`.
:::

# (Exercise)
- **EXERCISE**: Write pseudo-code to compute a divergence or a gradient.
...
To creating and destroying of grids is handled by the routine `refine`:
```fortran
subroutine refine
  use amr_commons
  implicit none
...
```
All the relevant routines can be found in the file `amr/refine_utils.f90`:
* `make_grid_coarse`: create 1 new grid at level 1
* `kill_grid`: destroy a grid
...
After this, the disconnected grids are added to the free memory linked list.
...
The strategy to set up boundary conditions in RAMSES is based on using “ghost regions” outside the computational domain, where flow variables are carefully specified in order to mimic the effect of the chosen type of boundary. While we go deeper into the subject of boundary condition in a later section, we explain here how boundary grids are positioned with respect to the main computational domain.
