...
`dtdiff_params=1d10`  | array of `real` | If `dt_control=.true.`, the global timestep is set as dtnew(ilevel)=dtdiff_params(1)*dtdiff_params(2)**nstep_coarse
...
|`Tray_min=0.5d0`       |`real` | Minimum temperature of the radiativve energy. `eray_min=aR*Tray_min=0.5d0**4` is computed and is used for radiative energies as `small_r` is used for density.
|`energy_fix=.false.`   | `logical`| Advect internal energy as an extra variable, automaticlally put at index `nvar`. PUT a +1 to NVAR in the Makefile if you activate `energy_fix`.
