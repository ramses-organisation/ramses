...
!------------------------------------------------------------------------
  ...
  use rt_parameters, ONLY :rt_nsource,rt_source_type,rt_src_x_center,rt_src_y_center &
       &,rt_src_length_x,rt_src_length_y,rt_exp_source,rt_src_group,rt_n_source 
  use radiation_parameters
...
!------------------------------------------------------------------------
  ...
  use rt_parameters, ONLY :rt_nsource,rt_source_type,rt_src_x_center,rt_src_y_center,rt_src_z_center &
       &,rt_src_length_x,rt_src_length_y,rt_src_length_z,rt_exp_source,rt_src_group,rt_n_source
  use radiation_parameters
  use hydro_commons, ONLY: nvar,uold
        ...
        do i=1,nn
           ...
           if(r .gt. 0.) then
              ...
              if(stellar_photon)then
                 ...
                 do igrp=1,ngrp
#if NDIM>1
                    lum_group = radiation_source(Tstar,igrp)/(scale_d*scale_v**2)*(pi*rstar_adim**2*c_cgs/scale_v)/(2.d0*Rin)
#endif
#if NDIM>2
                    lum_group = radiation_source(Tstar,igrp)/(scale_d*scale_v**2)*(pi*rstar_adim**2*c_cgs/scale_v)
#endif
