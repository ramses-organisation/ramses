...
subroutine condinit(x,u,dx,nn)
  ...
  real(dp)::mag_norm=3  ! strength of the magnetic field
  real(dp)::Bx=1        ! multiplyer magn field strength for x-component
  ...
  ! ........
  do i=1,nn
    x(i,1) = x(i,1) - 0.5d0 * boxlen
    x(i,2) = x(i,2) - 0.5d0 * boxlen
    x(i,3) = x(i,3) - 0.5d0 * boxlen
 
    ! density
    ! exponential profile along z
    q(i,1) = dens0 * exp(-x(i,3)**2 / (2.*height0**2))

    ! pressure via constant temperature
    ! ideal gas: P = n kB T
    q(i,5) =  q(i,1) * (kB*temp0/(mu_gas*mH))/scale_v**2
    ...
    ! exponential profile along z
    !Bx component 
    q(i,6     ) = Bx * mag_norm * exp(-x(i,3)**2/(2.*height0**2))
    q(i,nvar+1) = q(i,6)

    !By component
    q(i,7     ) = By * mag_norm * exp(-x(i,3)**2/(2.*height0**2))
    q(i,nvar+2) = q(i,7)

    !Bz component
    q(i,8     ) = Bz * mag_norm * exp(-x(i,3)**2/(2.*height0**2))
    q(i,nvar+3) = q(i,8)
#endif
