...
im7 = ax[2,0].imshow(np.log10(T_tot1), origin="lower", aspect='equal', extent=[ymin, ymax, zmin, zmax],interpolation='none')
im8 = ax[2,1].imshow(np.log10(T_tot2), origin="lower", aspect='equal', extent=[xmin, xmax, zmin, zmax],interpolation='none')
...
fig.savefig('fb.png') # => sometimes produces a less blurry figure

to_check={}
to_check["pressure"]=z5.flatten()  # data["data"]["pressure"]
to_check["density"]=z1.flatten() # data["data"]["density"]
to_check["temperature"]=temp.flatten() # data["data"]["density"]
...
visu_ramses.check_solution(to_check, 'cosmo',tolerance=tolerance)#,overwrite=True)