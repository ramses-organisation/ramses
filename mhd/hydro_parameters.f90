module hydro_parameters
  ...
  ! keep solver switch but add a flag to turn on/off
  ! activate_switch_solv=False by default
  ...
  ! modif nimhd
  ! move to nimhd_parameters module
  integer:: nxx=1
  integer:: nyy=2
  integer:: nzz=3
...
! THIS SEEMS TO BE BETTER for oblique choc
 integer :: magohm=1 !remove: Useful but practically never used
...
! Making a test or not Yes=1 No=0
  ...
  real(dp):: xmion=29.*1.667d-24 !remove ! mp*mu_ion of the most abundant ion (HCO+)
...
! IF NEW ADDED MODIFY units.f90 TO CONVERT IN USER UNITS
  ...
  real(dp):: rhoi0=1d0 !remove
  
  logical :: use_nonideal_mhd
  real(dp)::nu_sts=0.001 !remove
! fin modif nimhd
