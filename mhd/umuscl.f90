...
#endif
#if NDIM==2
     call trace2d(qin,bf,dbf,qm,qp,qRT,qRB,qLT,qLB,dx,dy   ,dt,ngrid)
#endif
#if NDIM==3
     call trace3d(qin,bf,dbf,qm,qp,qRT,qRB,qLT,qLB,dx,dy,dz,dt,ngrid)
#endif
  ...
  do k = ku1, ku2
     do j = ju1, ju2
        do i = iu1, iu2
           ...
           if(energy_fix) then
              do l = 1, ngrid
                 q(l,i,j,k,neul)=MAX((gamma-one)*uin(l,i,j,k,ieint),smallp)
              end do
           else
              do l = 1, ngrid
                 etot = uin(l,i,j,k,neul) - emag(l) -erad(l)
                 eint = etot/q(l,i,j,k,1)-eken(l)
                 q(l,i,j,k,neul)=MAX((gamma-one)*q(l,i,j,k,1)*eint,smallp)
              end do
           end if
