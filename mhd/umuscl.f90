        ...
        do j = jlo, jhi
            do i = ilo, ihi
                do l = 1, ngrid
                    ...
                    zero_flux = one
                    IF (ischeme /= 1) THEN
                        if (allow_switch_solver .and. ((iriemann == 2) .or. (iriemann == 3))) then
                            ! check for high B**2/P
                            check_switch_solver = ( (qright(4) ** 2 + qright(6) ** 2 + qright(8) ** 2) / qright(2) > switch_solv_B ) .or. &
                                & ( (qleft(4) ** 2 + qleft(6) ** 2 + qleft(8) ** 2) / qleft(2)     > switch_solv_B )
                            ! check for strong density discontinuities
                            check_switch_solver = check_switch_solver .or. &
                                & ( qleft(1) / qright(1) > switch_solv_dens ) .or. &
                                & ( qright(1) / qleft(1) > switch_solv_dens )
                            ! check for very small densities
                            check_switch_solver = check_switch_solver .or. &
                                & ( MIN(qleft(1), qright(1)) < switch_solv_min_dens )
                        else
