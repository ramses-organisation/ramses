program amr2cut
  ...
  allocate(x(1:ncell),y(1:ncell),z(1:ncell),l(1:ncell),var(1:ncell,1:nvar))
  x=0D0
  y=0D0
  z=0D0
  l=0
  ...
  allocate(toto(1:nx,1:ny,1:nvar))
  do ivar=1,nvar
  do i=1,nx
  do j=1,ny
     ind=1+(i-1)+(j-1)*nx
     toto(i,j,ivar)=var(ind,ivar)
  end do
  end do
  end do
  ...
  if (filetype=='ascii')then
     ...
     do j=1,ny
        do i=1,nx
           ...
           write(20,'(20(1PE15.7,2X))')xx,yy,real(l(ind)),(toto(i,j,k),k=1,nvar)
        end do
        write(20,*) " "
     end do
     close(20)
