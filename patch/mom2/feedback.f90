...
subroutine feedbk(ind_grid,ind_part,ind_grid_part,ng,np,ilevel)
  ...
  real(dp),dimension(1:nvector),save::p_to_be
  logical::use_cic_fb
  integer::hra
...
!   p_SN=1.42*1d5*1d5*2d33/(scale_v*scale_d*scale_l**3)
        ...
        do j=1,np
           ...
           if(birth_time.lt.(current_time-t0))then
            !   pstarnew(indp(j))=pstarnew(indp(j))+p_SN*n_SN(j)*p_boost*min(1.0,(dx_min/r_cool/aexp)**(3.0/2.0))/dx_loc**3
                p_to_be(j)=p_SN*n_SN(j)*p_boost*min(1.0,(dx_min/r_cool/aexp)**(3.0/2.0))/dx_loc**3
                ! write(*,*) "p_to_be:", p_to_be(j)
           endif
        end do
  ...
  if(momentum_feedback)then
    if(use_cic_fb)then
        ...
        call cic_amr_pstar(ind_cell,ind_part,ind_grid_part,x0,ng,np,ilevel,p_to_be)
    else
        do j=1,np
            pstarnew(indp(j))=pstarnew(indp(j))+p_to_be(j)
        enddo
    endif
  endif
...
!###########################################################
!###########################################################
subroutine cic_amr_pstar(ind_cell,ind_part,ind_grid_part,x0,ng,np,ilevel,p_to_be)
  use amr_commons
  use pm_commons
  use poisson_commons
  use hydro_commons, ONLY: pstarnew
  implicit none
  integer::ng,np,ilevel
  ...
  ! Particle-based arrays
  logical ,dimension(1:nvector),save::ok
  real(dp),dimension(1:nvector)::p_to_be
  ! Save type
...
!   end if
  ...
  do ind=1,twotondim
     ...
     do j=1,np
        ok(j)=(igrid(j,ind)>0.0).and.(p_to_be(j)>0.0)
     end do

     do j=1,np
        vol2(j)=p_to_be(j)*vol(j,ind)!/vol_loc
     end do

     do j=1,np
        if(ok(j))then
            ! write(*,*) "p,", p_to_be(j), vol(j,ind), vol_loc, vol2(j), pstarnew(indp(j,ind))
            pstarnew(indp(j,ind))=pstarnew(indp(j,ind))+vol2(j)
            ! write(*,*) "pstarnew", pstarnew(indp(j,ind))
        end if
     end do
