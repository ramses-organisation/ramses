...
subroutine condinit(x, u, dx, pert, nn)
    ...
    do i = 1, nn
        ...
        if (rr <= r1) then
            ...
            ! Convective zone
        else if ((rr > r1) .and. (rr < r2)) then
            T0 = 1 - ((gammainit2 - 1.0d0) / gammainit2) * g * (rho2 / p2) * (rr - r1)
            drho = 0.0d0
            if ((pert_r > 0.) .and. (pert_dx > 0.)) then
                if ((rr < r1 + pert_dx) .or. (rr > r2 - pert_dx)) then
...
subroutine eneana(x, e, dx, t, ncell, vol_heat, vol_cool, compute_volumes)
    ...
    if (compute_volumes .eqv..true.) then
        ...
        do i = 1, ncell
            ...
            rr = sqrt(rx ** 2 + ry ** 2 + rz ** 2)
            if ((rr > r1) .and. (rr < r1 + dr)) then
                if (t > t_heat_delay) then ! Delays the heating and cooling inputs by some time
                    ! heating
                    call compute_vol_energy(rr, r1, dr, e_norm)
                    e(i) = f1 * e_norm * min(1.0, (t - t_heat_delay) / t_heat_delay) ! erg/s/cm^3
                end if
            else if ((rr > r2 - dr) .and. (rr < r2)) then
                if (t > t_heat_delay) then
