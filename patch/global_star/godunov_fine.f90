...
subroutine add_gravity_source_terms(ilevel)
  ...
  do ind=1,twotondim
     ...
     do i=1,active(ilevel)%ngrid
        ...
        ! If strict equilibrium, we set rho_eq diff from zero
        ! Then we compute gravity source term in momentum equation
        ! rho'/rho g dt/2, where rho' = rho-rho_eq
...
subroutine godfine1(ind_grid,ncache,ilevel)
  ...
  !-----------------------------------------------
  call mag_unsplit(uloc,req_loc,peq_loc,gloc,flux,emfx,emfy,emfz,tmp,dx,dx,dx,dtnew(ilevel),ncache)
