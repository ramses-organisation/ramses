...
100                     continue
                ...
                do while (.not. eob)
                    ...
                    if (myid == 1) then
                        ...
                        do i = 1, nvector
                            ...
                            if (kpart >= header_part%npart) then
                                eob = .true.
                                exit
                            end if
                        end do
                    ...
                    do i = 1, jpart
                        ...
                        if (cc(i) == myid) then
                            ...
                            if (xx(i, 1) >= 0d0 .and. xx(i, 1) <= boxlen .and. &
                                & xx(i, 2) >= 0d0 .and. xx(i, 2) <= boxlen .and. &
                                & xx(i, 3) >= 0d0 .and. xx(i, 3) <= boxlen) then
                            ipart          = ipart + 1
                            if (ipart > npartmax) then
                                write(*, *) "Increase npartmax"
                                error = .true.
                    ...
                    do ibound = 1, header_amr%nboundary + header_amr%ncpu
                        ...
                        if (ncache > 0) then
                            ...
                            do idim = 1, header_amr%ndim
                                ...
                                if (size_blck /= ncache * sizeof(dummy_real)) then
                            ...
                            do ind = 1, header_amr%twotondim
                                ...
                                if (size_blck /= ncache * sizeof(dummy_int)) then
            ...
            if (icpu == 1) then
                ...
                do ivar = 1, nvar_min - 1
                    ...
                    if (restart_vars(ivar) > header_hydro%nvar) then
                        write(*, *) '[Error] ivar=', restart_vars(ivar),' is empty in the restart output'
                        call clean_stop
                    end if
                    if (restart_vars(ivar) < header_amr%ndim + 2) then
            ...
            do ilevel = 1, header_amr%nlevelmax
                ...
                do ibound = 1, header_amr%nboundary + header_amr%ncpu
                    ...
                    call MPI_BARRIER(MPI_COMM_WORLD, info)
#endif
                    if (ncache > 0 .and. ibound == icpu) then
                        kpart = 0
                        eob = .false.
                        do while (.not. eob)
                            xx    = 0.
                                ...
                                do i = 1, nvector / header_amr%twotondim
                                    ...
                                    do ind = 1, header_amr%twotondim
                                        ...
                                        if (son1 == 0) then
                                            ...
                                            do ivar = 1, nvar_min - 1
                                                if (restart_vars(ivar) > header_amr%ndim + 2 .and. restart_vars(ivar) <= header_hydro%nvar) then
    ...
    if (myid == 1) then
        ...
        icpu = icpu + 1
        if (icpu > header_part%ncpu) eocpu = .true.
    end if
#ifndef WITHOUTMPI
