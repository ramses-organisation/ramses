            ...
            do i = 1, active(ilevel)%ngrid
                ...
                if (uold(active(ilevel)%igrid(i) + iskip, 1) == 0.) then
                    if (ilevel > 1) then
                        in_grid = .true.
                        do idim = 1, ndim
                            in_grid = in_grid .and. (abs(xg(active(ilevel)%igrid(i), idim) + xc(ind, idim) - skip_loc(idim)) > restart_boxlen / 2.0)
                        end do
                        ...
                        if (in_grid) then
                            ...
                            if (ivar == 1) uold(active(ilevel)%igrid(i) + iskip, ivar) = IG_rho / scale_nH
                            if (ivar == ndim + 2) uold(active(ilevel)%igrid(i) + iskip, ivar) = IG_rho / scale_nH * IG_T2 / scale_T2 / (gamma - 1)
                            if (metal) then
                                if (ivar == imetal) uold(active(ilevel)%igrid(i) + iskip, ivar) = IG_rho / scale_nH * IG_metal
                            end if
                        end if
                    else
                        uold(active(ilevel)%igrid(i) + iskip, ivar) = 0.
        ...
        do jgrid = 1, numbl(icpu, ilevel)
            ...
            if (npart1 > 0) then
                ...
                do jpart = 1, npart1
                    ...
                    if (idp(ipart) == 1 .and. levelp(ipart) >= ilevel) then
                        npart2 = npart2 + 1
                    end if
