...
subroutine init_uold(ilevel)
    ...
    do ind = 1, twotondim
        ...
        do ivar = nvar, 1, - 1
            do i = 1, active(ilevel)%ngrid
                if (uold(active(ilevel)%igrid(i) + iskip, 1) < IG_rho / scale_nH) then
                    uold(active(ilevel)%igrid(i) + iskip, ivar)                      = 0D0
                    if (ivar == 1) uold(active(ilevel)%igrid(i) + iskip, ivar)        = max(IG_rho / scale_nH, smallr)
                    if (ivar == ndim + 2) then
                        uold(active(ilevel)%igrid(i) + iskip, ivar) = IG_T2 / scale_T2 / (gamma - 1) * max(IG_rho / scale_nH, smallr)
                    end if
                    if (metal) then
                        if (ivar == imetal) uold(active(ilevel)%igrid(i) + iskip, ivar) = max(IG_rho / scale_nH, smallr) * IG_metal
                    end if
                end if
            end do
        end do
...
subroutine init_gas_cic(ind_cell, ind_part, ind_grid_part, x0, ng, np, ilevel)
    ...
    do ind = 1, twotondim
        do j = 1, np
            if (nbors_father_cells(ind_grid_part(j), kg(j, ind)) > 0) then
    ...
    do ind = 1, twotondim
        ...
        do j = 1, np
            ok(j) = ok(j) .and. (idp(ind_part(j)) == 1)
        end do
...
subroutine init_gas_ngp(ind_grid, ind_part, ind_grid_part, ng, np, ilevel)
    ...
    do j = 1, np
        if (nbors_father_cells(ind_grid_part(j), kg(j)) > 0) then
...
subroutine mag_compute(ilevel)
    ...
    do i = 1, MAXGAL
        ...
        if (ic_mag_scale_B(i) /= 0.0) nogal = .false.
    end do
    if (nogal) return
