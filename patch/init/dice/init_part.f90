          ...
          do i=1,jpart
             ...
             if(cc(i)==myid)then
                ...
                if(.not.skip) then 
                   ...
                   levelp(ipart)  = levelmin
                   ! Particle type
                   ! Gadget uses 0 for gas, 1 for DM, and 2-4 for stars of various kind (disk, bulge, stars)
                   ! type_index = gadget type + 1
                   if (type_index .eq. 1) then ! gas
                      typep(ipart)%family = FAM_UNDEF
                   else if (type_index .eq. 2) then ! DM
                      typep(ipart)%family = FAM_DM
                   else ! stars
                      typep(ipart)%family = FAM_STAR
                   endif
                   ! Particle tag: 0 by default
                   typep(ipart)%tag = 0
                   ! Birth epoch
                   if(star) then
                      tp(ipart)    = tt(i)
