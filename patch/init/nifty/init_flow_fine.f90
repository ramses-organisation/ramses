        ...
        do ivar = nvar, 1, - 1
            do i = 1, active(ilevel)%ngrid
                if (uold(active(ilevel)%igrid(i) + iskip, 1) < IG_rho / scale_nH) then
                    uold(active(ilevel)%igrid(i) + iskip, ivar) = 0.
                    if (ivar == 1) uold(active(ilevel)%igrid(i) + iskip, ivar) = IG_rho
                    if (ivar == ndim + 2) uold(active(ilevel)%igrid(i) + iskip, ivar) = IG_rho * IG_T2 / scale_T2 / (gamma - 1)
                    if (metal) then
                        if (ivar == imetal) uold(active(ilevel)%igrid(i) + iskip, ivar) = IG_rho * IG_metal
                    end if
                end if
            end do
        end do
