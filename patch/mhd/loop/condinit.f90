...
subroutine condinit(x,u,dx,nn)
  ...
  real(dp)::xc,xr,xl,yl,yr,yc,al,ar,r0,a0
  ...
  do i=1,nn
     ...
     Ar = A0*max(R0-sqrt(xl**2+yr**2),0.0_dp)
     Al = A0*max(R0-sqrt(xl**2+yl**2),0.0_dp)
     q(i,6)=(Ar-Al)/dx
     Ar = A0*max(R0-sqrt(xr**2+yr**2),0.0_dp)
     Al = A0*max(R0-sqrt(xr**2+yl**2),0.0_dp)
     q(i,nvar+1)=(Ar-Al)/dx
     Ar = A0*max(R0-sqrt(xr**2+yl**2),0.0_dp)
     Al = A0*max(R0-sqrt(xl**2+yl**2),0.0_dp)
     q(i,7)=(Al-Ar)/dx
     Ar = A0*max(R0-sqrt(xr**2+yr**2),0.0_dp)
     Al = A0*max(R0-sqrt(xl**2+yr**2),0.0_dp)
     q(i,nvar+2)=(Al-Ar)/dx
     q(i,8)=0.0
     q(i,nvar+3)=0.0
  end do
