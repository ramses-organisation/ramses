...
contains
    ...
    subroutine init_force
        ...
        do jx = - kmax, kmax
            do jy = - kmax, kmax
                do jz = - kmax, kmax
                    if (float(jx ** 2 + jy ** 2 + jz ** 2) >= k1 ** 2 - 1e-5 .and. &
                        float(jx ** 2 + jy ** 2 + jz ** 2) <= k2 ** 2 + 1e-5) nmodes = nmodes + 1
                end do
            end do
        end do
        ...
        do jx = - kmax, kmax
            do jy = - kmax, kmax
                do jz = - kmax, kmax
                    if (float(jx ** 2 + jy ** 2 + jz ** 2) >= k1 ** 2 - 1e-5 .and. &
                        float(jx ** 2 + jy ** 2 + jz ** 2) <= k2 ** 2 + 1e-5) then
                    nmodes = nmodes + 1
...
subroutine update_random_forcing
    ...
    ! -------------------------------------------------------------------------------
    if (t /= tprev) then    ! prevent repeat at same t
        tprev = t

        if (myid == 1) then
            print *, 'forceit: t,t_turb=', t, t_turb
            if (t_turb <= t) print *, 'new force'
        end if
            ...
            do jx = - kmax, kmax
                ...
                do jy = - kmax, kmax
                    ...
                    do jz = - kmax, kmax
                        ...
                        if (fk >= k1 - 1e-5 .and. fk <= k2 + 1e-5) then
                            ...
                            if (do_helmh) then
                                ...
                                if (jx /= 0) lfxx = lfxx - a_helmh * corr * lkx
                                if (jy /= 0) lfyy = lfyy - a_helmh * corr * lky
                                if (jz /= 0) lfzz = lfzz - a_helmh * corr * lkz
                            end if
                            ! ------------------
                            j = j + 1
