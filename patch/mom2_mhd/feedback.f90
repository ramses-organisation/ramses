        ...
        do jgrid = 1, numbl(icpu, ilevel)
            ...
            if (npart1 > 0) then
                ...
                do jpart = 1, npart1
                    ...
                    if ( is_star(typep(ipart)) .and. tp(ipart) >= (current_time - t_sn_cont)) then
                        npart2 = npart2 + 1
                    end if
...
subroutine feedbk(ind_grid, ind_part, ind_grid_part, ng, np, ilevel)
    ...
    if (f_w == 0) then
        do j = 1, np
            ...
            if (birth_time < (current_time - t0)) then
                ...
                if (sf_log_properties .and. (n_SN(j) > 0.)) then
        ...
        if (momentum_feedback > 0) then
            ...
            do j = 1, np
                ...
                if (birth_time < (current_time - t0) .and. r_cool < (4.0 * dx_min / aexp)) then
