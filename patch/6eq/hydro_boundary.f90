...
subroutine make_boundary_hydro(ilevel)
    ...
    ! -------------------------------------------------------------------
    integer :: ibound, boundary_dir, idim, imat, inbor=1
    ...
    real(dp) :: switch, dx, dx_loc, scale, ekin, v
    real(dp), dimension(1:nvector) :: dtot
    real(dp), dimension(1:nvector, 1:nmat) :: g
    ...
    do ibound = 1, nboundary
        ...
        do igrid = 1, ncache, nvector
            ...
            do ind = 1, twotondim
                ...
                if ((boundary_type(ibound) / 10) == 0) then
                ...
                else if ((boundary_type(ibound) / 10) == 1) then
                    ...
                    if (no_inflow) then
                        ivar = 2 * nmat + gdim
                        if ((boundary_dir == 1) .or. (boundary_dir == 3) .or. (boundary_dir == 5)) then
                            do i = 1, ngrid
                                uold(ind_cell(i), ivar) = min(0d0, uold(ind_cell(i), ivar))
                            end do
                        end if
                        if ((boundary_dir == 2) .or. (boundary_dir == 4) .or. (boundary_dir == 6)) then
                            do i = 1, ngrid
