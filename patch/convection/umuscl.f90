...
! ----------------------------------------------------------------
! JRCC : Add req,peq in unsplit call
subroutine unsplit(uin,req,peq,gravin,pin,flux,tmp,dx,dy,dz,dt,ngrid)
  use amr_parameters
  use const
  ...
  ! JRCC : Declare qpeq "fluxes", i.e. qpeq(i,j,k) = peq(i,j,k) - peq(i-1,j,k)
  ...
  if(scheme=='muscl')then
    ...
    ! JRCC : add peq and qpeq to trace2d call
     call trace2d(qin,dq,qm,qp,req,peq,qpeq,dx,dy   ,dt,ngrid)
#endif
#if NDIM==3
...
#if NDIM>1
! JRCC : add peq to parameters
subroutine trace2d(q,dq,qm,qp,req,peq,qpeq,dx,dy,dt,ngrid)
  use amr_parameters
  use hydro_parameters
  use const
  implicit none
...
!              qp(l,i,j,k,ir,3) = max(smallr, qp(l,i,j,k,ir,3))
              ...
              qm(l,i,j,k,ip,3) = p - peq(l,i,j,k) + half*dpz + sp0*dtdz*half + qpeq(l,i,j,k,3)
...
!###########################################################
!###########################################################
! JRCC : Add qpeq to cmpflxm parameters
subroutine cmpflxm(qm,im1,im2,jm1,jm2,km1,km2, &
     &             qp,ip1,ip2,jp1,jp2,kp1,kp2, &
     &             qpeq, ilo,ihi,jlo,jhi,klo,khi, ln,lt1,lt2, &
     &            flx,tmp,ngrid)
     ! TODO add Peq with correct indices! Check for shifts and other stuff!
     ! TODO otherwise pass directly Peq with indices needed from call
  use amr_parameters
  use hydro_parameters
  use const
  implicit none
  ...
  ! JRCC : Add declaration of qpeq
  ! JRCC : Attention to indices: must be the same of left state
  real(dp),dimension(1:nvector,im1:im2,jm1:jm2,km1:km2,1:ndim)::qpeq
  ...
  do k = klo, khi
     do j = jlo, jhi
        do i = ilo, ihi
           ...
           ! Normal momentum
           do l = 1, ngrid
              ! JRCC : remove average of Peq at the interface
              flx(l,i,j,k,ln) = fgdnv(l,2) - qpeq(l,i,j,k,xdim)
              ! write(*,*) flx(l,i,j,k,ln), qleft(l,3), qright(l,3), qpeq(l,i,j,k,xdim)
           end do
...
!###########################################################
!###########################################################
! JRCC : Add req, peq in parameters
subroutine ctoprim(uin,q,c,req,peq,gravin,dt,ngrid)
  use amr_parameters
  use hydro_parameters
  use const
  implicit none
  ...
  real(dp),dimension(1:nvector,iu1:iu2,ju1:ju2,ku1:ku2,1:ndim)::gravin
  real(dp),dimension(1:nvector,iu1:iu2,ju1:ju2,ku1:ku2,1:nvar)::q
  ! JRCC : declaration of req, peq
  real(dp),dimension(1:nvector,iu1:iu2,ju1:ju2,ku1:ku2)::c,req,peq
     ...
     do j = ju1, ju2
        do i = iu1, iu2
           do l = 1, ngrid
              ...
              ! JRCC : compute P' and store it in q(ndim+2), P' = P - peq
              q(l,i,j,k,ndim+2) = q(l,i,j,k,ndim+2) - peq(l,i,j,k)

              ! Gravity predictor step
              ! JRCC : add factor in front of gravin -> (rho - rhoeq)/rho
              q(l,i,j,k,2) = q(l,i,j,k,2) + (q(l,i,j,k,1)-req(l,i,j,k))/q(l,i,j,k,1)*gravin(l,i,j,k,1)*dtxhalf
#if NDIM>1    
              ! JRCC : add factor in front of gravin -> (rho - rhoeq)/rho
              q(l,i,j,k,3) = q(l,i,j,k,3) + (q(l,i,j,k,1)-req(l,i,j,k))/q(l,i,j,k,1)*gravin(l,i,j,k,2)*dtxhalf
#endif
#if NDIM>2
              ! JRCC : add factor in front of gravin -> (rho - rhoeq)/rho 
              q(l,i,j,k,4) = q(l,i,j,k,4) + (q(l,i,j,k,1)-req(l,i,j,k))/q(l,i,j,k,1)*gravin(l,i,j,k,3)*dtxhalf
#endif
