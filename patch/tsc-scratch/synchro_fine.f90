...
subroutine sync(ind_grid, ind_part, ind_grid_part, ng, np, ilevel, xtondim)
    ...
    if ((accel_gr(1) /= 0) .or. (accel_gr(2) /= 0) .or. (accel_gr(3) /= 0)) then
        do idim = 1, ndim
            do j = 1, np
...
subroutine InitStoppingRate(nn, dt, indp, vol, v, nu, xtondim)
    ...
    if ((constant_t_stop) .and. (stopping_rate < 0.0)) then ! add a "constant_nu_stop" option so you can turn drag totally off.
        nu(1:nvector) = 1./ t_stop ! Or better yet, add pre-processor directives to turn drag off.
    else if ((constant_t_stop) .and. (stopping_rate >= 0.0)) then
