...
subroutine rho_ana(x,d,dx,ncell)
  ...
  case(-2)
     ! Point mass with Plummer softening (corresponding to gravana gravity_type==2)
     gmass=gravity_params(1) ! M (G=1 in c.u.)
     emass=dx
     ...
     do i=1,ncell
        ...
        d(i)=3d0*gmass/(4d0*pi) * emass**2/rr**5
     end do
  ...
  case(-3)
     ...
     a1_rho = a1*1d3/(Myr2sec**2) / (4d0*pi*factG_in_cgs) / z0 / scale_d
