...
!##############################################################
!##############################################################
subroutine getnborfather_acc(ind_cell,ind_father,ilevel, &
  ...
  integer::ilevel
  integer::ind_cell
  integer,dimension(0:twondim)::ind_father
  ...
  if(ilevel==1)then
     ...
     ! Get father cell
     ind_father(0)=ind_cell

     ix(3)=(ind_father(0)-1)/nxny
     ix(2)=(ind_father(0)-1-ix(3)*nxny)/nx
     ix(1)=(ind_father(0)-1-ix(2)*nx-ix(3)*nxny)


     do idim=1,ndim

        if(ix(idim)>0)then
           ind_father(2*idim-1)=ind_father(0)-iskip1(idim)
        else
           ind_father(2*idim-1)=ind_father(0)+iskip2(idim)
        end if

        if(ix(idim)<ibound(idim))then
           ind_father(2*idim)=ind_father(0)+iskip1(idim)
        else
           ind_father(2*idim)=ind_father(0)-iskip2(idim)
        end if
     ...
     ! Get father cell position in the grid
     pos=(ind_father(0)-ncoarse-1)/ngridmax+1
     
     ! Get father grid
     ind_grid_father=ind_father(0)-ncoarse-(pos-1)*ngridmax

     ! Get neighboring father grids
     ! Store central grid
     igridn(0)=ind_grid_father
     ! Store neighboring grids
     do j=1,twondim
         igridn(j)=son(nbor(ind_grid_father,j))
     end do

     ! Get neighboring cells for selected cells
     ! Reset indices
     icelln_ok(1:twondim)=0
     ! Compute cell numbers
     do i=1,twondim
        if(igridn(ggg_acc(pos,i))>0)then
           icelln_ok(i)=ncoarse+(hhh_acc(pos,i)-1)*ngridmax+igridn(ggg_acc(pos,i))
        end if
     end do

     ! Update neighboring father cells for selected cells
     do j=1,twondim
        if(icelln_ok(j)>0)then
           ind_father(j)=icelln_ok(j)
        else
           ind_father(j)=nbor(ind_grid_father,j)
        end if
     end do
     
  end if

end subroutine getnborfather_acc
!##############################################################
!##############################################################
!##############################################################
!##############################################################
subroutine get3cubefather_godfine(ind_cell_father,nbors_father_cells,&
     &                    nbors_father_grids,ncell,ilevel)
  use amr_commons 
  implicit none
  integer::ncell,ilevel
...
end subroutine get3cubefather_godfine
