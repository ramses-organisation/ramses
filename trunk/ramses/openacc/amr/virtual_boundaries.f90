subroutine acc_communication_buffers(ilevel)
  use amr_commons
  implicit none
  integer :: ilevel
  integer :: icpu,i

  !$acc data present(nemisspe_acc, nrecpe_acc, reception, emission) &
  !$acc      present(sum_nemisspe, sum_nrecpe)

  !$acc kernels
  do icpu=1,ncpu
  do i   =ilevel,nlevelmax
     nemisspe_acc(icpu,i) = emission(icpu,i)%ngrid*twotondim
     nrecpe_acc  (icpu,i) = reception(icpu,i)%ngrid*twotondim
  end do
  end do

  do icpu=1,ncpu
  do i   =ilevel,nlevelmax
     sum_nemisspe(icpu,i) = sum(nemisspe_acc(1:icpu,i))
     sum_nrecpe  (icpu,i) = sum(nrecpe_acc  (1:icpu,i))
  end do
  end do
  !$acc end kernels
...
end subroutine acc_communication_buffers

subroutine update_globalvar_dp_to_host(xx,ilevel)
   use amr_commons
   use hydro_commons
   use acc_commons
   use poisson_commons
   ...
   integer::ilevel
   !----------------------------------------------------------------------------------
   ! copy vars :: uold, phi, f, phi_old to host for a specific level
   ! optimized data transfer
   !----------------------------------------------------------------------------------
   integer::ngrid,ncell,igrid,i,j,k,iskip,cell,icpu
   real(dp),dimension(:),allocatable::tmp_xx
   ...
   allocate(grid(1:ngrid),tmp_xx(1:ncell))
   
   !$acc data create(grid,tmp_xx) present(xx,active,reception)
   ...
   do igrid=1,ngrid
      ...
      tmp_xx((i-1)*ngrid+igrid) = xx(cell)
   end do
   end do

   !$acc update host(tmp_xx)
   
   ! Unpacking
   do i=1,twotondim
   do igrid=1,active(ilevel)%ngrid
      iskip = ncoarse + (i-1)*ngridmax 
      cell = iskip + active(ilevel)%igrid(igrid)
      xx(cell) = tmp_xx((i-1)*ngrid+igrid)
   end do
   end do
   ...
   do igrid=1,reception(icpu,ilevel)%ngrid
      iskip = ncoarse + (i-1)*ngridmax 
      cell = iskip + reception(icpu,ilevel)%igrid(igrid)
      xx(cell) = tmp_xx((i-1)*ngrid+active(ilevel)%ngrid+sum(reception(1:icpu,ilevel)%ngrid)-reception(icpu,ilevel)%ngrid+igrid)
   end do
   end do
   end do
   ...
   deallocate(grid,tmp_xx)
   
end subroutine update_globalvar_dp_to_host
!################################################################ 
!################################################################
!################################################################
!################################################################
subroutine update_globalvar_int_to_host(xx,ilevel)
   use amr_commons
   use hydro_commons
   use acc_commons
   use poisson_commons
   ...
   integer::ilevel
   !----------------------------------------------------------------------------------
   ! copy flag2 to host (for other AMR vars need further development)
   !----------------------------------------------------------------------------------
   integer::ngrid,ncell,igrid,i,j,k,iskip,cell,icpu
   real(dp),dimension(:),allocatable::tmp_xx
   integer,dimension(:),allocatable::grid
...
end subroutine update_globalvar_int_to_host
!################################################################ 
!################################################################
!################################################################
!################################################################
subroutine update_globalvar_dp_to_device(xx,ilevel)
   use amr_commons
   use hydro_commons
   use acc_commons
   use poisson_commons
   ...
   integer::ilevel
   !----------------------------------------------------------------------------------
   ! copy vars :: uold, phi, f, phi_old to device for a specific level
   ! optimized data transfer
   !----------------------------------------------------------------------------------
   integer::ngrid,ncell,igrid,i,j,k,iskip,cell,icpu
   real(dp),dimension(:),allocatable::tmp_xx
   ...
   do igrid=1,reception(icpu,ilevel)%ngrid
      iskip = ncoarse + (i-1)*ngridmax 
      cell = iskip + reception(icpu,ilevel)%igrid(igrid)
      tmp_xx((i-1)*ngrid+active(ilevel)%ngrid+sum(reception(1:icpu,ilevel)%ngrid)-reception(icpu,ilevel)%ngrid+igrid) = xx(cell)
   end do
   end do
   end do

   !$acc update device(tmp_xx)
...
end subroutine update_globalvar_dp_to_device
!################################################################ 
!################################################################
!################################################################
!################################################################
subroutine update_globalvar_int_to_device(xx,ilevel)
   use amr_commons
   use hydro_commons
   use acc_commons
   use poisson_commons
   ...
   integer::ilevel
   !----------------------------------------------------------------------------------
   ! copy flag2 to device for a specific level
   ! optimized data transfer  (for other AMR vars need further development)
   !----------------------------------------------------------------------------------
   integer::ngrid,ncell,igrid,i,j,k,iskip,cell,icpu
   real(dp),dimension(:),allocatable::tmp_xx
   integer,dimension(:),allocatable::grid
...
end subroutine update_globalvar_int_to_device
!################################################################ 
!################################################################
!################################################################
!################################################################
...
!################################################################
!################################################################
subroutine make_virtual_fine_dp_acc(xx,ilevel)
  use amr_commons
  implicit none
#ifndef WITHOUTMPI
#ifndef INCLUDEOK
#define INCLUDEOK
  ...
  integer::info,buf_count,tag=101
  integer,dimension(ncpu)::reqsend,reqrecv
  real(dp),dimension(:),allocatable :: rectot   ! reception total
  real(dp),dimension(:),allocatable :: emisstot ! emission total
  integer::nrectot,nemisstot
  integer, dimension(:), allocatable::nemisspe,nrecpe   ! number of elements to send & recv
  integer, dimension(:), allocatable::emissdisp,recdisp ! displacements for the big arrays
...
#undef INCLUDEOK
  ...
  allocate(nemisspe(ncpu))
  allocate(nrecpe(ncpu))
  allocate(emissdisp(ncpu))
  allocate(recdisp(ncpu))

  ! find the total size of reception and emission at this level
  nrectot=0
  nemisstot=0
  do icpu=1,ncpu
     nrectot=nrectot+reception(icpu,ilevel)%ngrid*twotondim
     nemisstot=nemisstot+emission(icpu,ilevel)%ngrid*twotondim
     ! number of elements to send and receive per pe
     nemisspe(icpu) = emission(icpu,ilevel)%ngrid*twotondim
     nrecpe(icpu) = reception(icpu,ilevel)%ngrid*twotondim
     ! displacements (start from zero)
     emissdisp(icpu) = sum(nemisspe(1:icpu))-nemisspe(icpu)
     recdisp(icpu)   = sum(nrecpe(1:icpu))-nrecpe(icpu)
  end do

  ! allocate global arrays for all to all
  allocate(rectot(1:nrectot))
  allocate(emisstot(1:nemisstot))

  !$acc data present(xx, reception, emission, nemisspe_acc, nrecpe_acc) &
  !$acc      create(rectot, emisstot) & 
  !$acc      present(sum_nemisspe,sum_nrecpe)
  ...
  do icpu=1,ncpu
     ...
     if(ncache>0) then
       countrecv=countrecv+1
       !$acc host_data use_device(rectot)
       call MPI_IRECV(rectot(recdisp(icpu)+1),nrecpe(icpu), &
            & MPI_DOUBLE_PRECISION,icpu-1,tag,MPI_COMM_WORLD,reqrecv(countrecv),info)
       !$acc end host_data
     end if
  end do

  ngrid_maxval = maxval(emission(:,ilevel)%ngrid)
  ...
  do i=1,ngrid_maxval
     if (i.GT.emission(icpu,ilevel)%ngrid) cycle     
     iskip=ncoarse+(j-1)*ngridmax
     emisstot(sum_nemisspe(icpu,ilevel)-nemisspe_acc(icpu,ilevel)+(j-1)*emission(icpu,ilevel)%ngrid+i)=xx(emission(icpu,ilevel)%igrid(i)+iskip)
  end do
  end do
  !$acc end parallel
  end do
  ...
  do icpu=1,ncpu
     ...
     if(ncache>0) then
       countsend=countsend+1
       !$acc host_data use_device(emisstot)
       call MPI_ISEND(emisstot(emissdisp(icpu)+1),nemisspe(icpu), &
            & MPI_DOUBLE_PRECISION,icpu-1,tag,MPI_COMM_WORLD,reqsend(countsend),info)
       !$acc end host_data
     end if
  end do
  ...
  !$acc parallel async(icpu)
     if (reception(icpu,ilevel)%ngrid>0) then
        ...
        do i=1,reception(icpu,ilevel)%ngrid
           iskip=ncoarse+(j-1)*ngridmax
           xx(reception(icpu,ilevel)%igrid(i)+iskip)=rectot(sum(nrecpe_acc(1:icpu,ilevel))-nrecpe_acc(icpu,ilevel)+(j-1)*reception(icpu,ilevel)%ngrid+i)
        end do
        end do
     end if
  !$acc end parallel
...
#ifdef MPROFILING
  tt2=MPI_WTIME(info)
  acc_t_make_virtual_fine_dp = acc_t_make_virtual_fine_dp + (tt2-tt1)
#endif

111 format('   Entering make_virtual_fine for level ',I2)

end subroutine make_virtual_fine_dp_acc
!################################################################
!################################################################
!################################################################
!################################################################
...
!################################################################
!################################################################
subroutine make_virtual_reverse_dp_acc(xx,ilevel)
  use amr_commons
  implicit none
#ifndef WITHOUTMPI
#ifndef INCLUDEOK
...
#undef INCLUDEOK
  ...
  nemisstot=0
  do icpu=1,ncpu
     nrectot=nrectot+emission(icpu,ilevel)%ngrid*twotondim
     nemisstot=nemisstot+reception(icpu,ilevel)%ngrid*twotondim
     ! number of elements to send and receive per pe
     nemisspe(icpu) = reception(icpu,ilevel)%ngrid*twotondim
     nrecpe(icpu) = emission(icpu,ilevel)%ngrid*twotondim
     ! displacements (start from zero)
     emissdisp(icpu) = sum(nemisspe(1:icpu))-nemisspe(icpu)
     recdisp(icpu)   = sum(nrecpe(1:icpu))-nrecpe(icpu)
  end do
  ...
  !$acc data present(xx, reception, emission) &
  !$acc copyin(nemisspe, nrecpe) create(rectot, emisstot)

  if(ilevel.LE.switchlevel)then 

     print*,"----------------------------------->> Modify the code and recompile it!"
     call clean_stop
  ...
  !$acc parallel async(icpu)
     if (reception(icpu,ilevel)%ngrid>0) then
     !$acc loop collapse(2) worker vector independent
     do j=1,twotondim
     do i=1,reception(icpu,ilevel)%ngrid
        iskip=ncoarse+(j-1)*ngridmax
        emisstot(sum(nemisspe(1:icpu))-nemisspe(icpu)+(j-1)*reception(icpu,ilevel)%ngrid+i)=xx(reception(icpu,ilevel)%igrid(i)+iskip)
     end do
     end do
     end if
  !$acc end parallel
  ...
  do icpu=1,ncpu
     ...
     do i=1,emission(icpu,ilevel)%ngrid
        iskip=ncoarse+(j-1)*ngridmax
        xx(emission(icpu,ilevel)%igrid(i)+iskip) = &
      & xx(emission(icpu,ilevel)%igrid(i)+iskip) + rectot(sum(nrecpe(1:icpu))-nrecpe(icpu)+(j-1)*emission(icpu,ilevel)%ngrid+i)
     end do
     end do
     end if
  end do
...
#ifdef MPROFILING
  tt2=MPI_WTIME(info)
  acc_t_make_virtual_reverse_dp = acc_t_make_virtual_reverse_dp + (tt2-tt1)
#endif

111 format('   Entering make_virtual_reverse for level ',I2)

end subroutine make_virtual_reverse_dp_acc
!################################################################
!################################################################
!################################################################
!################################################################
