...
!================================================================
!================================================================
subroutine hilbert3d_acc(x,y,z,order,bit_length,npoint)
  use amr_parameters, ONLY: qdp
  use acc_commons, only: state_diagram
  implicit none

  integer::bit_length,npoint
  integer,dimension(1:npoint)::x,y,z
  integer,dimension(1:npoint)::order
  ...
  !$acc present(x,y,z,order,state_diagram) private(i_bit_mask)
  do ip=1,npoint
     ...
     do i=bit_length-1,0,-1
        ...
        i_bit_mask(3*i+2)=btest(x(ip),i)
        i_bit_mask(3*i+1)=btest(y(ip),i)
        i_bit_mask(3*i  )=btest(z(ip),i)
        ...
        if(i_bit_mask(3*i+2)) order(ip)=order(ip)+2**(3*i+2)
        if(i_bit_mask(3*i+1)) order(ip)=order(ip)+2**(3*i+1)
        if(i_bit_mask(3*i  )) order(ip)=order(ip)+2**(3*i)
