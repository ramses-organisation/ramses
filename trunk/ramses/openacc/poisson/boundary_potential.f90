...
subroutine make_boundary_phi(ilevel)
  ...
  !$acc data present(phi) &
  !$acc copyin(boundary,ind_grid,ind_cell,xc,xx,rr,skip_loc,multipole,pp)
  !$acc update device(xg)
  
  ! Loop over boundaries
  do ibound=1,nboundary
  !$acc parallel async(ibound)
     ! Loop over grids by vector sweeps
     ncache=boundary(ibound,ilevel)%ngrid
...
#ifdef MPROFILING
  tt2=MPI_WTIME(info)
  acc_t_make_boundary_phi = acc_t_make_boundary_phi + (tt2-tt1)
#endif
...
subroutine make_boundary_mask(ilevel)
  ...
  !$acc data present(f) copyin(boundary,ind_grid,ind_cell)
...
#ifdef MPROFILING
  tt2=MPI_WTIME(info)
  acc_t_make_boundary_mask = acc_t_make_boundary_mask + (tt2-tt1)
#endif
