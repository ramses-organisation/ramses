...
subroutine make_initial_phi(ilevel,icount)
  ...
  integer::igrid,ncache,i,ind,iskip,idim,ibound, nbin,ibin
  real(dp),dimension(1:nvector,1:twotondim),save::phi_int

  real(dp),dimension(:,:,:),allocatable::phi_int_aux
  integer ,dimension(:,:)  ,allocatable::ind_grid,ind_cell,ind_cell_father
  integer ,dimension(:)    ,allocatable::ngrid
#ifdef MPROFILING
  include 'mpif.h'
  integer::info
  real(kind=8)::tt1,tt2
  ...
  allocate(phi_int_aux(1:nbin,1:nvector,1:twotondim))
  allocate(ind_grid(1:nbin,1:nvector),ind_cell(1:nbin,1:nvector), &
         & ind_cell_father(1:nbin,1:nvector))
  allocate(ngrid(1:nbin))

  ! First kernel dedicated to intepolation (CPU)
  do igrid=1,ncache,nvector
     ibin = int(igrid/nvector)+1
     ngrid(ibin)=MIN(nvector,ncache-igrid+1)

     do i=1,ngrid(ibin)
        ind_grid(ibin,i)=active(ilevel)%igrid(igrid+i-1)
        ind_cell_father(ibin,i)=father(ind_grid(ibin,i))
     end do
     ...
     if(ilevel==1)then
        ...
        ! Interpolate
        call interpol_phi(ind_cell_father(ibin,:),phi_int,ngrid(ibin),ilevel,icount)
        phi_int_aux(ibin,:,:) = phi_int
     end if

  end do
  ! End loop over grids

  !$acc data present(phi,f,active) & 
  !$acc      copyin(phi_int_aux,ind_grid,ind_cell,ind_cell_father,ngrid)

  ! Second kernel (GPU)
  ! Loop over myid grids by vector sweeps
  !$acc parallel loop vector independent
  do igrid=1,ncache,nvector
     ibin = int(igrid/nvector)+1
     ...
     if(ilevel==1)then
        ...
        do ind=1,twotondim
           iskip=ncoarse+(ind-1)*ngridmax
           do i=1,ngrid(ibin)
              ind_cell(ibin,i)=iskip+ind_grid(ibin,i)
           end do
           do i=1,ngrid(ibin)
              phi(ind_cell(ibin,i))=0.0d0
           end do
           do idim=1,ndim
              do i=1,ngrid(ibin)
                 f(ind_cell(ibin,i),idim)=0.0
              end do
           end do
        end do
        ! End loop over cells
        ...
        ! Interpolate (in the first kernel)
          ! With this way we decouple the inlining and leave it
          ! for the CPU. Decompose the original kernel into 2, 
          ! one for the CPU and the other for the GPU   
        ...
        do ind=1,twotondim
           ...
           do i=1,ngrid(ibin)
              phi(ind_cell(ibin,i))=phi_int_aux(ibin,i,ind)
           end do
           do idim=1,ndim
              do i=1,ngrid(ibin)
                 f(ind_cell(ibin,i),idim)=0.0
  ...
  deallocate(phi_int_aux)
  deallocate(ind_grid,ind_cell,ind_cell_father)
  
#ifdef MPROFILING
  tt2=MPI_WTIME(info)
  acc_t_make_initial_phi = acc_t_make_initial_phi + (tt2-tt1)
#endif
...
subroutine make_multipole_phi(ilevel)
  ...
  !$acc data present(phi,active) &
  !$acc copyin(ind_grid,ind_cell,xc,xx,rr,skip_loc,multipole,pp)
...
! ****** In our approach the simple_boundary is false *******
! If true this part will have issues with race conditions and so
! one has to apply auxiliary arrays, atomic directive and the routine directive
...
#ifdef MPROFILING
  tt2=MPI_WTIME(info)
  acc_t_make_multipole_phi = acc_t_make_multipole_phi + (tt2-tt1)
#endif
