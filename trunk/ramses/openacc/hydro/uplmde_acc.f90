...
!###########################################################
#if NDIM>2
subroutine tracexyz_gpu_2d(q,dq,c,qm,qp,dx,dy,dz,dt,ngrid,nxp,ncube)
  use amr_parameters
  use hydro_parameters
  use acc_parameters
  use const
  ...
  integer ::ngrid,nxp,ncube
  real(dp)::dx,dy,dz, dt

  real(dp),dimension(-1:nxp+2,-1:nxp+2,-1:nxp+2,1:nvar,1:ncube)::q  
  real(dp),dimension(-1:nxp+2,-1:nxp+2,-1:nxp+2,1:nvar,1:ndim,1:ncube)::dq 
  real(dp),dimension(-1:nxp+2,-1:nxp+2,-1:nxp+2,1:nvar,1:ndim,1:ncube)::qm 
  real(dp),dimension(-1:nxp+2,-1:nxp+2,-1:nxp+2,1:nvar,1:ndim,1:ncube)::qp 
  real(dp),dimension(-1:nxp+2,-1:nxp+2,-1:nxp+2,1:ncube)::c  
...
!$acc data present(q,dq,c,qm,qp,dtdx,dtdy,dtdz)
     ...
     do j = 0,nxp+1
        do i = 0,nxp+1
           do l = 1, ngrid
              ...
              qp(i,j,k,ir,1,nc) = r + (apright+amright+azrright)     +srx
              qp(i,j,k,iu,1,nc) = u + (apright-amright         )*cc/r+sux
              qp(i,j,k,ip,1,nc) = p + (apright+amright         )*csq +spx
              qp(i,j,k,iv,1,nc) = v + (                azvright)     +svx
              qp(i,j,k,iw,1,nc) = w + (                azwright)     +swx
              ...
              qm(i,j,k,ir,1,nc) = r + (apleft+amleft+azrleft)     +srx
              qm(i,j,k,iu,1,nc) = u + (apleft-amleft        )*cc/r+sux
              qm(i,j,k,ip,1,nc) = p + (apleft+amleft        )*csq +spx
              qm(i,j,k,iv,1,nc) = v + (              azvleft)     +svx
              qm(i,j,k,iw,1,nc) = w + (              azwleft)     +swx
              ...
              qp(i,j,k,ir,2,nc) = r + (apright+amright+azrright)     +sry
              qp(i,j,k,iv,2,nc) = v + (apright-amright         )*cc/r+svy
              qp(i,j,k,ip,2,nc) = p + (apright+amright         )*csq +spy
              qp(i,j,k,iu,2,nc) = u + (                azuright)     +suy
              qp(i,j,k,iw,2,nc) = w + (                azwright)     +swy
              ...
              qm(i,j,k,ir,2,nc) = r + (apleft+amleft+azrleft)     +sry
              qm(i,j,k,iv,2,nc) = v + (apleft-amleft        )*cc/r+svy
              qm(i,j,k,ip,2,nc) = p + (apleft+amleft        )*csq +spy
              qm(i,j,k,iu,2,nc) = u + (              azuleft)     +suy
              qm(i,j,k,iw,2,nc) = w + (              azwleft)     +swy
              ...
              qp(i,j,k,ir,3,nc) = r + (apright+amright+azrright)     +srz
              qp(i,j,k,iw,3,nc) = w + (apright-amright         )*cc/r+swz
              qp(i,j,k,ip,3,nc) = p + (apright+amright         )*csq +spz
              qp(i,j,k,iu,3,nc) = u + (                azuright)     +suz
              qp(i,j,k,iv,3,nc) = v + (                azvright)     +svz
              ...
              qm(i,j,k,ir,3,nc) = r + (apleft+amleft+azrleft)     +srz
              qm(i,j,k,iw,3,nc) = w + (apleft-amleft        )*cc/r+swz
              qm(i,j,k,ip,3,nc) = p + (apleft+amleft        )*csq +spz
              qm(i,j,k,iu,3,nc) = u + (              azuleft)     +suz
              qm(i,j,k,iv,3,nc) = v + (              azvleft)     +svz
...
!$acc parallel loop collapse(5) gang vector !async(3)
  ...
  do n = ndim+3, nvar
     do k = 0,nxp+1
        do j = 0,nxp+1
           do i = 0,nxp+1
              a   =  q(i,j,k,n,nc)    ! Cell centered values
              u   =  q(i,j,k,iu,nc)
              v   =  q(i,j,k,iv,nc)
              w   =  q(i,j,k,iw,nc)
              ...
              if(u>zero)spzero=-project_out
              azaright = half*(-one-spzero )*dax
              qp(i,j,k,n,1,nc) = a + azaright + sax
              
              ! Left state
              spzero = (u    )*dtdx
              if(u<=zero)spzero=+project_out
              azaleft = half*(+one-spzero )*dax
              qm(i,j,k,n,1,nc) = a + azaleft + sax

              ! Top state
              spzero = (v    )*dtdy
              if(v>zero)spzero=-project_out
              azaright = half*(-one-spzero )*day
              qp(i,j,k,n,2,nc) = a + azaright + say
              
              ! Bottom state
              spzero = (v    )*dtdy
              if(v<=zero)spzero=+project_out
              azaleft = half*(+one-spzero )*day
              qm(i,j,k,n,2,nc) = a + azaleft + say
              
              ! Front state
              spzero = (w    )*dtdy
              if(w>zero)spzero=-project_out
              azaright = half*(-one-spzero )*daz
              qp(i,j,k,n,3,nc) = a + azaright + saz
              
              ! Back state
              spzero = (w    )*dtdy
              if(w<=zero)spzero=+project_out
              azaleft = half*(+one-spzero )*daz
              qm(i,j,k,n,3,nc) = a + azaleft + saz
           end do
        end do
     end do
  end do
