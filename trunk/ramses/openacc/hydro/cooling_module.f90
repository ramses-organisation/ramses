...
!                               COOLING MODULE
!=============================================================================
! The subroutines and the functions of general interest are:
!
! ROUTINES THAT ARE TO BE CALLED BY THE HYDRO CODE:
!
!    subroutine set_model(...) : 
!          to choose the cooling model and its parameters
!
!    subroutine set_table(aexp) : 
!          to create the table with the default parameters
!          More practical to be called than:
!          cmp_table(nH_min,nH_max,T2_min,T2_max,nbin_n,nbin_T,aexp)
!
!    subroutine solve_cooling(...) : 
!          to compute the cooling
!
! ROUTINE TO BE MODIFIED IF NECESSARY:
!
!    function J0simple(aexp) : 
!          gives the J0 as a function of the redshift according to the 
!          Teyssier or Theuns models
!
!=============================================================================
module cooling_module
  ...
  ! The default parameters of the table
  ...
  ! One MUST HAVE OR teyssier OR theuns OR madau
...
!=======================================================================
subroutine solve_cooling(nH,T2,Zsolar,boost,dtcool,delta_T2,ncell)
!=======================================================================
  implicit none  
  integer::ncell
  real(kind=8)::dtcool
  real(kind=8),dimension(1:ncell)::nH,T2,delta_T2,Zsolar,boost
    
  real(kind=8)::facT,dlog_nH,dlog_T2,coeff,precoeff,h,h2,h3
  real(kind=8)::metal_sc,cool,heat,cool_com,heat_com,w1T,w2T,w11,w12,w21,w22,err,yy,yy2,yy3
  ...
  real(kind=8)::fa,fb,fprimea,fprimeb,alpha_sc,beta_sc,gamma_sc
  ...
  integer::tau_negative, inegative
  real(kind=8)::t_negative
  !-----------------------------------------------------------------------------
#ifndef DERIVED_DATATYPES
  ! Auxiliary variables added to avoid explicit reference to derived data types
  ! when inside parallel regions (OpenACC does not work with derived data types)
  real(kind=8),dimension(table%n1,table%n2)::table_cool,table_cool_prime,table_heat,table_heat_prime
  real(kind=8),dimension(table%n1,table%n2)::table_cool_com,table_cool_com_prime,table_heat_com,table_heat_com_prime
  real(kind=8),dimension(table%n1,table%n2)::table_metal,table_metal_prime
  real(kind=8),dimension(1:table%n1)::table_nH
#endif
  !-----------------------------------------------------------------------------
...
#ifndef DERIVED_DATATYPES
  table_cool = table%cool
  table_cool_prime = table%cool_prime
  table_heat = table%heat
  table_heat_prime = table%heat_prime

  table_cool_com = table%cool_com
  table_cool_com_prime = table%cool_com_prime
  table_heat_com = table%heat_com
  table_heat_com_prime = table%heat_com_prime
  
  table_metal = table%metal
  table_metal_prime = table%metal_prime 
  ...
  !-------------------------------------
  !          Start data region 
  !$acc data copyin(table_cool,table_cool_prime,table_heat,table_heat_prime) &
  !$acc copyin (table_cool_com,table_cool_com_prime,table_heat_com) &
  !$acc copyin (table_heat_com_prime,table_metal,table_metal_prime) &
  !$acc pcreate(tau_old, time_old) &
  !$acc pcreate(zzz,facH,i_nH,w1H,w2H,tau,tau_ini,time_max,time,wmax,ind) &
  !$acc present(nH,T2,Zsolar,boost)
  !-------------------------------------
  !$acc parallel loop gang vector present(nH,boost)
  do i=1,ncell
     zzz(i)=Zsolar(i)
...
#else
     w1H(i)=(table_nH(i_nH(i)+1)-facH(i))*dlog_nH
     w2H(i)=(facH(i)-table_nH(i_nH(i)  ))*dlog_nH
#endif
     tau(i)=T2(i)
     tau_ini(i)=T2(i)
     time_max(i)=dtcool*precoeff*nH(i)
     time(i)=0d0
  ...
  ! Check positivity 
  tau_negative=0
  !$acc parallel loop reduction(+:tau_negative)
  do i=1,ncell
     if(tau(i)>0.) cycle
       tau_negative=1
  end do
  ...
  ! print*, "tau_negative initial = ", tau_negative 
 
  if (tau_negative/=0) then
  ...
  do i=1,ncell
     ...
     ! (this is taken care of by each thread)  
     do while (time(ind(i))<time_max(ind(i)))

    !TODO: restructure this check
    ! iter=iter+1
    ! if (iter > 500) then
...
#else
           fa=table_cool(i_nH(ind(i)),i_T2  )*w1H(ind(i))+table_cool(i_nH(ind(i))+1,i_T2  )*w2H(ind(i))
           fb=table_cool(i_nH(ind(i)),i_T2+1)*w1H(ind(i))+table_cool(i_nH(ind(i))+1,i_T2+1)*w2H(ind(i))
           fprimea=table_cool_prime(i_nH(ind(i)),i_T2  )*w1H(ind(i))+table_cool_prime(i_nH(ind(i))+1,i_T2  )*w2H(ind(i))
           fprimeb=table_cool_prime(i_nH(ind(i)),i_T2+1)*w1H(ind(i))+table_cool_prime(i_nH(ind(i))+1,i_T2+1)*w2H(ind(i))
#endif
           alpha_sc=fprimea
           beta_sc=3d0*(fb-fa)/h2-(2d0*fprimea+fprimeb)/h
           gamma_sc=(fprimea+fprimeb)/h2-2d0*(fb-fa)/h3
           cool=10d0**(fa+alpha_sc*yy+beta_sc*yy2+gamma_sc*yy3)
           cool_prime=cool/tau(ind(i))*(alpha_sc+2d0*beta_sc*yy+3d0*gamma_sc*yy2)
...
#else
           fa=table_heat(i_nH(ind(i)),i_T2  )*w1H(ind(i))+table_heat(i_nH(ind(i))+1,i_T2  )*w2H(ind(i))
           fb=table_heat(i_nH(ind(i)),i_T2+1)*w1H(ind(i))+table_heat(i_nH(ind(i))+1,i_T2+1)*w2H(ind(i))
           fprimea=table_heat_prime(i_nH(ind(i)),i_T2  )*w1H(ind(i))+table_heat_prime(i_nH(ind(i))+1,i_T2  )*w2H(ind(i))
           fprimeb=table_heat_prime(i_nH(ind(i)),i_T2+1)*w1H(ind(i))+table_heat_prime(i_nH(ind(i))+1,i_T2+1)*w2H(ind(i))
#endif
           alpha_sc=fprimea
           beta_sc=3d0*(fb-fa)/h2-(2d0*fprimea+fprimeb)/h
           gamma_sc=(fprimea+fprimeb)/h2-2d0*(fb-fa)/h3
           heat=10d0**(fa+alpha_sc*yy+beta_sc*yy2+gamma_sc*yy3)
           heat_prime=heat/tau(ind(i))*(alpha_sc+2d0*beta_sc*yy+3d0*gamma_sc*yy2)
...
#else
           fa=table_cool_com(i_nH(ind(i)),i_T2  )*w1H(ind(i))+table_cool_com(i_nH(ind(i))+1,i_T2  )*w2H(ind(i))
           fb=table_cool_com(i_nH(ind(i)),i_T2+1)*w1H(ind(i))+table_cool_com(i_nH(ind(i))+1,i_T2+1)*w2H(ind(i))
           fprimea=table_cool_com_prime(i_nH(ind(i)),i_T2  )*w1H(ind(i))+table_cool_com_prime(i_nH(ind(i))+1,i_T2  )*w2H(ind(i))
           fprimeb=table_cool_com_prime(i_nH(ind(i)),i_T2+1)*w1H(ind(i))+table_cool_com_prime(i_nH(ind(i))+1,i_T2+1)*w2H(ind(i))
#endif
           alpha_sc=fprimea
           beta_sc=3d0*(fb-fa)/h2-(2d0*fprimea+fprimeb)/h
           gamma_sc=(fprimea+fprimeb)/h2-2d0*(fb-fa)/h3
           cool_com=10d0**(fa+alpha_sc*yy+beta_sc*yy2+gamma_sc*yy3)
           cool_com_prime=cool_com/tau(ind(i))*(alpha_sc+2d0*beta_sc*yy+3d0*gamma_sc*yy2)
...
#else
           fa=table_heat_com(i_nH(ind(i)),i_T2  )*w1H(ind(i))+table_heat_com(i_nH(ind(i))+1,i_T2  )*w2H(ind(i))
           fb=table_heat_com(i_nH(ind(i)),i_T2+1)*w1H(ind(i))+table_heat_com(i_nH(ind(i))+1,i_T2+1)*w2H(ind(i))
           fprimea=table_heat_com_prime(i_nH(ind(i)),i_T2  )*w1H(ind(i))+table_heat_com_prime(i_nH(ind(i))+1,i_T2  )*w2H(ind(i))
           fprimeb=table_heat_com_prime(i_nH(ind(i)),i_T2+1)*w1H(ind(i))+table_heat_com_prime(i_nH(ind(i))+1,i_T2+1)*w2H(ind(i))
#endif
           alpha_sc=fprimea
           beta_sc=3d0*(fb-fa)/h2-(2d0*fprimea+fprimeb)/h
           gamma_sc=(fprimea+fprimeb)/h2-2d0*(fb-fa)/h3
           heat_com=10d0**(fa+alpha_sc*yy+beta_sc*yy2+gamma_sc*yy3)
           heat_com_prime=heat_com/tau(ind(i))*(alpha_sc+2d0*beta_sc*yy+3d0*gamma_sc*yy2)
...
#else
           fa=table_metal(i_nH(ind(i)),i_T2  )*w1H(ind(i))+table_metal(i_nH(ind(i))+1,i_T2  )*w2H(ind(i))
           fb=table_metal(i_nH(ind(i)),i_T2+1)*w1H(ind(i))+table_metal(i_nH(ind(i))+1,i_T2+1)*w2H(ind(i))
           fprimea=table_metal_prime(i_nH(ind(i)),i_T2  )*w1H(ind(i))+table_metal_prime(i_nH(ind(i))+1,i_T2  )*w2H(ind(i))
           fprimeb=table_metal_prime(i_nH(ind(i)),i_T2+1)*w1H(ind(i))+table_metal_prime(i_nH(ind(i))+1,i_T2+1)*w2H(ind(i))
#endif
           alpha_sc=fprimea
           beta_sc=3d0*(fb-fa)/h2-(2d0*fprimea+fprimeb)/h
           gamma_sc=(fprimea+fprimeb)/h2-2d0*(fb-fa)/h3
           metal_sc=10d0**(fa+alpha_sc*yy+beta_sc*yy2+gamma_sc*yy3)
           metal_prime=metal_sc/tau(ind(i))*(alpha_sc+2d0*beta_sc*yy+3d0*gamma_sc*yy2)

           ! Total net cooling
           lambda=cool+zzz(ind(i))*metal_sc-heat+(cool_com-heat_com)/nH(ind(i))
  ...
  ! Check positivity 
  tau_negative=0
  !$acc parallel loop gang vector reduction(+:tau_negative)
  do i=1,ncell
     if(tau(i)>0.) cycle
       tau_negative=1
  end do
  ...
  ! print*, "tau_negative final = ", tau_negative 
  ...
  ! Compute delta T
  !$acc parallel loop gang vector present(tau, tau_ini, delta_T2)
  do i=1,ncell
     delta_T2(i)=tau(i)-tau_ini(i)
  end do
  !$acc end parallel loop 
