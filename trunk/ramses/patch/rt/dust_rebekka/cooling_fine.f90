...
subroutine coolfine1(ind_grid,ngrid,ilevel)
  ...
  use rt_cooling_module, only: rt_solve_cooling,iIR,rt_isIRtrap &
       ,rt_pressBoost,iIRtrapVar,kappaSc,a_r,is_kIR_T,rt_vc,AGN_RT
#endif
  implicit none
  ...
  ! XXX
  real(dp),dimension(1:nvector),save:: prefact
  real(dp),dimension(nIons),save:: xion2
  real(dp),dimension(1:nvector),save :: Temp
  real(dp) :: Toff, TT, uold2
  real(kind=8) :: mu, TK 
#endif
  ...
  do ind=1,twotondim
     ...
     if(rt_vc) then      ! Add/remove radiation work on gas. Eq A6 in RT15
        ...
        do i=1,nleaf 
           ...
           !!! XXX change Rebekka
           !!if (AGN_RT) then ! Calculate kappaSc
           !!    ! If running with rt_vc=.true. this needs to be tested again
           !!    kScIR  = kScIR * prefact(i) 
           !!    !!Toff = 1.d3      ! [K]
           !!    !!if(Temp(i) > Toff) kScIR = 1.e-5
           !!    !!kScIR  = kScIR  * exp(-Temp(il)/Toff)
           !!end if
           !!! XXX
	...
	gr_energy(i) = MAX(gr_energy(i),gr_floor(i))
     ...
     ! XXX 
     do i = 1,nleaf
        mu= 1./(X*(1.+xion(i,1)) + 0.25*Y*(1.+xion(i,2)+2.*xion(i,3)))
        TK = T2(i) * mu
        Toff = 1.d3
        if(TK > Toff)then
           prefact(i)=1.0d-5
