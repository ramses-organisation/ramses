...
module rt_cooling_module
  ...
  public rt_set_model, rt_solve_cooling, update_UVrates, cmp_chem_eq     &
         ...
         , rt_isoPress, rt_T_rad, rt_vc, a_r, AGN_RT
...
contains
  ...
  !-----------------------------------------------------------------------
    ...
    if(rt .and. nGroups .gt. 0) then
       ...
       ! XXX
       if (AGN_RT) then ! Calculate kappaAbs, kappaSc
           kAbs_loc = kAbs_loc * prefact
           kSc_loc  = kSc_loc  * prefact
           !!Temp = T2(icell) * mu   ! [K]
           !!if(Temp>Toff)then
           !!  kAbs_loc = 1e-5
           !!  kSc_loc = 1e-5
           !!endif
           !!kAbs_loc = kAbs_loc * exp(-Temp/Toff)
           !!kSc_loc  = kSc_loc  * exp(-Temp/Toff)
       end if
       ! XXX
...
#if NGROUPS>0 
    if(rt_isIR) then
       if(kAbs_loc(iIR) .gt. 1d-10 .and. .not. rt_T_rad) then
          ...
          E_rad = group_egy_erg(iIR) * dNp(iIR)
          !write(*,*)'DBG',rt_c_cgs,E_rad,c_cgs,a_r,TK,kAbs_loc(iIR),Zsolar(icell),rho,ddt(icell),one_over_C_v
