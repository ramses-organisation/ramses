...
subroutine accrete_sink(ind_grid,ind_part,ind_grid_part,ng,np,ilevel,on_creation)
  ...
  use rt_parameters,only: nGroups, iGroups, group_egy, rt_AGN2, group_agnegy_frac
#endif
  implicit none
  integer::ng,np,ilevel
  ...
  integer::i,j,nx_loc,isink,ivar,irad,idim,ind,mygroup
  real(dp)::v2,d,e,d_floor,density,volume
#ifdef SOLVERmhd
  real(dp)::bx1,bx2,by1,by2,bz1,bz2
#endif
  real(dp),dimension(1:nvar)::z
  real(dp)::factG,scale_nH,scale_T2,scale_l,scale_d,scale_t,scale_v,scale_Np,scale_Fp
           ...
           if (on_creation)then
              ...
              if (flux_accretion .or. bondi_accretion)then
                 if (feedback_scheme=='energy')then
                    share_en = 1.0
                    share_mo = 0.0
                 else if (feedback_scheme=='momentum')then
                    share_en = 0.0
                    share_mo = 1.0
                 else if (feedback_scheme=='radiation')then
                    share_en = 0.0
                    share_mo = 0.0
                 else 
                    share_en = 0.5
                 ...
                 fbk_ener_AGN=share_en*min(delta_mass(isink)*T2_AGN/scale_T2*weight/volume*d/density,T2_max/scale_T2*weight*d)
                 fbk_mom_AGN=share_mo*min(delta_mass(isink)*v_AGN*(180./cone_opening)*1.e5/scale_v*weight/volume*d/density,v_max*1.e5/scale_v*weight*d)
              end if
              ...
              if(agn)then
                 ...
                 if(rt_AGN2)then
                    ...
                    do mygroup=1,nGroups
                       Np_inj=dert * group_agnegy_frac(mygroup) / (scale_evtocode*group_egy(mygroup)) / (vol_loc*scale_vol) / scale_Np*weight/volume
                       rtunew(indp(j,ind),iGroups(mygroup))=rtunew(indp(j,ind),iGroups(mygroup))+Np_inj
                    enddo
                 else
                    if(ok_blast_agn(isink).and.delta_mass(isink)>0.0)then
                      if(feedback_scheme=='energy'.or.feedback_scheme=='mix')then
                        unew(indp(j,ind),5)=unew(indp(j,ind),5)+fbk_ener_AGN/vol_loc
                    end if
                  
                    if(feedback_scheme=='momentum'.or.feedback_scheme=='mix')then
...
subroutine compute_accretion_rate(write_sinks)
  ...
  use hydro_commons
#ifdef RT
  use rt_parameters,only: rt_AGN2
#endif
  implicit none
#ifndef WITHOUTMPI
  include 'mpif.h'
...
subroutine quenching(ilevel)
  ...
  do i=1,active(ilevel)%ngrid
     ...
     if(npart2.gt.0)then
        ...
        ave_v=ave_v/tot_m
        ave_w=ave_w/tot_m
        sig_u=sqrt(abs(sig_u/tot_m-ave_u**2))*scale_v/1d5
        sig_v=sqrt(abs(sig_v/tot_m-ave_v**2))*scale_v/1d5
        sig_w=sqrt(abs(sig_w/tot_m-ave_w**2))*scale_v/1d5
     ...
     do ind=1,twotondim
        ...
        !!!if(.not.(str_d>2000.0.and.MAX(sig_u,sig_v,sig_w)>100.))then
           flag2(ind_cell)=0 ! unflag formation if condition met
        !else
        !   flag2(ind_cell)=1
        end if
     end do
     ...
     if (msink(isink)>-1)then
        do jsink=isink+1,nsink
           ...
           if (merge)then
              ...
              if (agn.and.feedback_scheme=='radiation') then ! 10*R_Schwarzschild
                 acc_lum(1:nsink)=1e-4*acc_rate(1:nsink)*msink(1:nsink)/(10*2*factG*msink(1:nsink)/(3e8*scale_v)**2)
              end if
