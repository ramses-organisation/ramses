...
subroutine condinit(x,u,dx,nn)
  ...
  !================================================================
  integer::ivar,i,it,nticks,outticks,ind_gal
  ...
  real(dp)::dxmin,Al,Ar,ztick,rltick,rrtick,twothird,zavg,Bscale,r_scale,z_scale
...
!  a2=T2_star / scale_T2 ! sound speed squared
  ...
  do i=1,nn
    ...
    ! Magnetic field, circular, prop. to rho**(2/3)
    ! average exp(-h) over the z axis
    r_scale = twothird / typ_mag_radius
    z_scale = twothird / typ_mag_height

    ztick = xx(3) + 0.5*(dxmin-dx)
    zavg = 0.0
    do it=1,nticks
      zavg=zavg + exp(-ABS(ztick)*z_scale)
      ztick=ztick + dxmin
    end do
    Bscale = B_ave * zavg/DBLE(nticks) / r_scale / dx

    ! B left
    ! X direction
    rltick = SQRT( (xx(1)- 0.5*dx)**2 + (xx(2)- 0.5*dx)**2 )
    rrtick = SQRT( (xx(1)- 0.5*dx)**2 + (xx(2)+ 0.5*dx)**2 )
