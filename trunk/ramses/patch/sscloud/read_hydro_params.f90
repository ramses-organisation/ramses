subroutine read_hydro_params(nml_ok)
  ...
  namelist/physics_params/cooling,haardt_madau,metal,isothermal,bondi &
       ...
       & ,units_density,units_time,units_length,ir_feedback
...
105 continue
  ...
  ! momentum tracking.
  !---------------------------------------
  if(bondi .and. sink_angular_momentum)then
     if (myid==1)write(*,*)'sink angular momentum is not tracked when using bondi accretion.'
     call clean_stop
  endif
