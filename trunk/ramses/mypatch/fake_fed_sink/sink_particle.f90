        ...
        do ii=-2*ir_cloud,2*ir_cloud
           ...
           if(rr>0.and.rr<=rmax)then
              ...
              do j=1,np
                 ...
                 if (rr<=rmass .and. (.not. nbody_sink))then
...
!remove        if (ilevel<nlevelmax)write(*,*),'trying to accrete from cell which is not at levelmax...'
        ...
        if (new_born_all(isink)==0)then 
           ...
           !        c2=MAX(gamma*(gamma-1.0)*ethermal,smallc**2)a
           ...
           do ii=1,8
              do jj=1,8
                 do kk=1,8
                    xpoint(1)=xx(1)+((ii-0.5)*0.125-0.5)*dx_min
                    xpoint(2)=xx(2)+((jj-0.5)*0.125-0.5)*dx_min
                    xpoint(3)=xx(3)+((kk-0.5)*0.125-0.5)*dx_min
                    r_rel_s=xpoint(1:3)-xsink(isink,1:3)
                    v_rel_s=vv(1:3)-vsink(isink,1:3)
                    l_spec2=sum(cross(r_rel_s,v_rel_s)**2)
                    e_spec=0.5*sum(v_rel_s(1:3)**2)-(msink(isink)+mgas)/(sum(r_rel_s(1:3)**2)**0.5+tiny(0.0_dp))
                    r_min=-1.*(msink(isink)+mgas)/(2.*e_spec)*(1.-(1.+(2.*l_spec2*e_spec)/((msink(isink)+mgas)**2))**0.5)
                    if (r_min/dx_min<0.25 .and. e_spec<0.)count_points=count_points+1.
                 end do
              end do
           end do
           count_points=count_points/(8.**3)
...
!                 acc_mass=acc_mass*count_points
!                 if (e_spec>0 .or. r_min>0.25*dx_min)acc_mass=0.
                 !use floor
                 acc_mass=min(acc_mass,(d-d_floor)*vol_loc)
              end if
...
subroutine compute_accretion_rate(write_sinks)
  ...
  real(dp)::r2,v2,c2,density,volume,ethermal,dx_min,scale,mgas,rho_inf
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  if (nbody_sink)call f_sink_sink
  ...
  do isink=1,nsink
     ...
     if (.not. nbody_sink)fsink(isink,1:ndim)=0.
     ...
     if (nbody_sink)then
        if (level_sink(isink)==0)then
           ...
           !for nbody_sink, the sinks are advanced at the finest level timestep
...
subroutine f_gas_sink(ilevel)
 ...
 integer::ilevel
  ...
  real(dp)::dx,dx_loc,scale,dx_min,vol_loc
  real(dp),dimension(1:twotondim,1:3)::xc
  real(dp),dimension(1:3)::skip_loc
  ...
  real(dp),dimension(1:nvector)::d2,mgas
  ...
  do isink=1,nsink
     ...
     do igrid=1,ncache,nvector
        ...
        do ind=1,twotondim
           ...
           !gas mass in cell
           do i=1,ngrid
              mgas(i)=uold(ind_cell(i),1)*vol_loc
           end do
           ...
           do i=1,ngrid
              ff(i,1:ndim)=mgas(i)*msink(isink)/(ssoft**2+d2(i))**1.5*ff(i,1:ndim)
           end do
           !add gas acceleration due to sink
           do i=1,ngrid
              f(ind_cell(i),1:ndim)=f(ind_cell(i),1:ndim)+ff(i,1:ndim)/mgas(i)
           end do
           !change potential due to sink (correct for BOXLEN???)
!           do i=1,ngrid
...
subroutine f_sink_sink
  ...
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  integer::nx_loc,isink,idim,jsink
