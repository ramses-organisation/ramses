...
!     call clump_finder(.false.)
     call make_sink_from_clump(levelmin)
     !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! 
...
subroutine make_sink_from_clump(ilevel)
  ...
  integer ::ntot,ntot_all,info,pr,maxid
  logical ::ok_free
  ...
  real(dp)::dx,dx_loc,scale,vol_loc,dx_min,vol_min
  real(dp)::bx1,bx2,by1,by2,bz1,bz2
  real(dp),dimension(1:nvector,1:3)::xtest,pos
  ...
  logical ,dimension(1:nvector),save::ok,ok_new=.true.,ok_true=.true.
  integer ,dimension(1:ncpu)::ntot_sink_cpu,ntot_sink_all
  integer ,dimension(1:nvector)::cc,cell_lev,cell_index
  ...
  if (nstep_coarse==0) then
     do pr=1,ncpu
        if (myid == pr) then
           open(10, FILE = 'sink_pos.txt')
           do ii=1,100
              read(10,*),pos(1,1),pos(1,2),pos(1,3)
              call cmp_cpumap(pos,cc,1)
              if (cc(1) .eq. myid)then
                 call get_cell_index(cell_index,cell_lev,pos,ilevel,1)
  ...
  if(numbtot(1,ilevel)>0)then
     ...
     do igrid=1,ncache,nvector
        ...
        do ind=1,twotondim
           ...
           nnew=0
           do i=1,ngrid
              if (ind_cell(i)>maxid)maxid=ind_cell(i)
           ...
           do i=1,nnew
              ...
              msink_new(index_sink)=1.d-20 !min((d-d_thres)*vol_loc,msink_max2)
