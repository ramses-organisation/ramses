...
subroutine move1(ind_grid,ind_part,ind_grid_part,ng,np,ilevel)
 ...
 ! Compute parent cell position
  ...
  if(sink)then
     ...
     !update the particle velocity (should not be used, but you newer know...)
     !for example in newdt_fine ...
     do idim=1,ndim
        do j=1,np
           isink=-idp(ind_part(j))
           if(isink>0)then
  ...
  do idim=1,ndim
     if(static)then
        ...
        do j=1,np
           isink=-idp(ind_part(j))
           if(isink>0)new_xp(j,idim)=xp(ind_part(j),idim)+vsnew(isink,idim,ilevel)*dtnew(ilevel)
        end do
     else
        do j=1,np
...
integer::lev,isink
  ...
  if (myid==1)print*,'fsink',fsink(1:2,1:ndim)
