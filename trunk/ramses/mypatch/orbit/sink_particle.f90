...
!        acc_rate(1:nsinkmax)=acc_rate(1:nsinkmax)/dtnew(levelmin)
        if(myid==1.and.nsink>0)then
           ...
           write(*,*)'Number of sink = ',nsink
           write(*,'(" ========================================================================================================================================= ")')
           write(*,'("  Id     M[Msol]    x           y           z           vx          vy          vz         |x|           |v|")')
           write(*,'(" ========================================================================================================================================= ")')
           do i=nsink,1,-1
              isink=idsink_sort(i)
              l_abs=(lsink(isink,1)**2+lsink(isink,2)**2+lsink(isink,3)**2)**0.5+1.d-99
              rot_period=32*3.1415*msink(isink)*(dx_min)**2/(5*l_abs)
              write(*,'(I5,2X,F9.5,3(2X,F10.8),3(2X,F10.8),2X,F10.8,2X,F10.8)')idsink(isink),msink(isink)*scale_m/1.98892d33, &
                   xsink(isink,1:ndim),vsink(isink,1:ndim),((xsink(isink,1)-0.5)**2.+(xsink(isink,2)-0.5)**2.+(xsink(isink,3)-0.5)**2.)**0.5,&
                   (vsink(isink,1)**2.+vsink(isink,2)**2.+vsink(isink,3)**2.)**0.5
           end do
           write(*,'(" ========================================================================================================================================= ")')
        endif
        acc_rate=0.
...
subroutine accrete_jeans(ind_grid,ind_part,ind_grid_part,ng,np,ilevel)
  ...
  do j=1,np
     if(ok(j))then
        ...
        xx(1)=(xg(ind_grid_part(j),1)+xc(icell(j),1)-skip_loc(1))*scale
        xx(2)=(xg(ind_grid_part(j),2)+xc(icell(j),2)-skip_loc(2))*scale
        xx(3)=(xg(ind_grid_part(j),3)+xc(icell(j),3)-skip_loc(3))*scale
...
subroutine make_sink_from_clump(ilevel)
  ...
  idsink(2)=2
  vsink=0.
  msink(1:2)=2.*(2*ACOS(0.0))**2*(3.2**(-3.))
  if (myid==1)print*,'msink in user units',msink(1)
  tsink(1:2)=birth_epoch
  xsink(1,1)=0.5+2./128.
  xsink(2,1)=0.5-2./128.
  xsink(1:2,2)=0.5
  xsink(1:2,3)=0.5
  vsink(1,2)=2.1190386637722107!3.1415926/3.2
  vsink(2,2)=-2.1190386637722107!-3.1415926/3.2
  level_sink(1:2)=0
  nsink=2
  nindsink=2
...
! compute hessian
  ...
  !determinant
  det=hess(1,1)*hess(2,2)*hess(3,3)+hess(1,2)*hess(2,3)*hess(3,1)+hess(2,1)*hess(3,2)*hess(1,3) &
  ...
  do i=1,3
     do j=1,3
        ...
        do k=1,3
           check_inv(i,j)=check_inv(i,j)+hess(i,k)*minor(k,j)/det
        end do
     end do
  end do

  if ((displacement(1)**2+displacement(2)**2+displacement(3)**2)**0.5/dx_loc > 1.)then
     write(*,*)'displacement to true maximum is too big and therefore shortened to dx.'
     displacement(1)=dx_loc*displacement(1)/(displacement(1)**2+displacement(2)**2+displacement(3)**2)**0.5
     displacement(2)=dx_loc*displacement(2)/(displacement(1)**2+displacement(2)**2+displacement(3)**2)**0.5
     displacement(3)=dx_loc*displacement(3)/(displacement(1)**2+displacement(2)**2+displacement(3)**2)**0.5
  end if
