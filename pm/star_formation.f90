...
subroutine star_formation(ilevel)
    ...
    if (sf_log_properties .and. ifout > 1) then
        ...
        if ((.not. file_exist) .or. (abs(t - trestart) < dtnew(ilevel))) then
            ...
            if (sf_virial) then
                do i = 1, ngrid
                    ...
                    if (ok(i)) then
                        ...
                        if (sf_tdiss > 0d0) then
                            if (sf_compressive) then
                                tdec = sf_tdiss * dx_loc / sqrt(uold(ind_cell(i), ivirial1) + uold(ind_cell(i), ivirial2))
                                if (uold(ind_cell(i), ivirial1) > 0d0) uold(ind_cell(i), ivirial1) = uold(ind_cell(i), ivirial1) * exp(- dtold(ilevel) / tdec)
                                if (uold(ind_cell(i), ivirial2) > 0d0) uold(ind_cell(i), ivirial2) = uold(ind_cell(i), ivirial2) * exp(- dtold(ilevel) / tdec)
                            else
