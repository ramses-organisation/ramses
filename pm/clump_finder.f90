...
subroutine clump_finder(create_output, keep_alive)
    ...
    if (myid == 1) then
        if (ntest_all > 0 .and. clinfo) then
    ...
    if (npeaks_tot > 0) then
        ...
        if (clinfo .and. saddle_threshold <= 0) call write_clump_properties(.false.)
...
subroutine propagate_flag(nmove, nzero)
    ...
    do ipart = 1, ntest
        jpart = testp_sort(ipart)
        if (imaxp(jpart) /= - 1) then
            if (flag2(icellp(jpart)) /= flag2(imaxp(jpart))) nmove = nmove + 1
            flag2(icellp(jpart)) = flag2(imaxp(jpart))
            if (flag2(icellp(jpart)) == 0) nzero = nzero + 1
        end if
    end do
...
subroutine neighborsearch(xx, ind_cell, ind_max, np, count, ilevel, action)
    ...
    do j = 1, np
        ...
        if (action >= 4) clump_nr(j) = flag2(ind_cell(j)) ! save clump number
    end do
            ...
            if (npart1 > 0) then
                ...
                do jpart = 1, npart1
                    ...
                    if (age_cut_clfind > 0d0 .and. star .and. use_proper_time) then
                        if ((is_star(typep(ipart))) .and. (t - tp(ipart) < age_cut_clfind) .and. (tp(ipart) /= 0d0)) then
                            npart2 = npart2 + 1
                        end if
                        ! All particles
                    else
    ...
    do itestcell = 1, ntest ! loop over all test cells
        ...
        if (global_peak_id /= 0) then
            ...
            do ipart = 1, prtcls_in_grid
                ...
                j = 0
                k = 0
                if (xg(grid, 1) - xp(this_part, 1) / boxlen + (nx - 1) / 2.0 <= 0) i = 1
                if (xg(grid, 2) - xp(this_part, 2) / boxlen + (ny - 1) / 2.0 <= 0) j = 1
                if (xg(grid, 3) - xp(this_part, 3) / boxlen + (nz - 1) / 2.0 <= 0) k = 1
