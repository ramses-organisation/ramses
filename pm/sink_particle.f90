...
subroutine create_cloud_from_sink
                    ...
                    do isink = 1, nsink
                        ...
                        if (cc(1) == myid) then
                            ...
                            if (rr <= rmass) then
                                ...
                                if (mass_sink_direct_force >= 0.0) then
    ...
    if (mass_sink_direct_force >= 0.0) then
        do isink = 1, nsink
            direct_force_sink(isink) = (msink(isink) >= mass_sink_direct_force * M_sun / (scale_d * scale_l ** ndim))
        end do
    else
        do isink = 1, nsink
                ...
                if ( .not. on_creation) then
                    if (agn) then
                        if (ok_blast_agn(isink) .and. delta_mass(isink) > 0.0) then
                            if (AGN_fbk_frac_ener > 0.0) then ! thermal AGN feedback
                                unew(indp(j, ind), ndim + 2) = unew(indp(j, ind), ndim + 2) + fbk_ener_AGN / vol_loc
                            end if

                            if (AGN_fbk_frac_mom > 0.0) then ! momentum AGN feedback
                                ...
                                if (orth_dist <= abs(cone_dist) * tan_theta) then
    ...
    do isink = 1, nsink
        ...
        if (agn .and. dMsink_overdt(isink) > 0.0) then
            ...
            if (T2_min <= 0.0) then ! If zero or less, we always deposit feedback
                ...
                if ((T2_gas >= T2_min) .or. (delta_mass(isink) >= mgas * (T2_min - T2_gas) / (T2_AGN - T2_min))) then
                    ok_blast_agn(isink) = .true.
                end if
            end if
        end if

        if (msink(isink) >= max_mass_nsc * M_sun / scale_m .and. mass_smbh_seed > 0.0) dMsink_overdt(isink) = 0.0
        ...
        if (msink(isink) > 0.) then
            do jsink = isink + 1, nsink
                ...
                if (overlap) then
                    ...
                    if (mass_merger_vel_check > 0 .and. (msink(isink) + msink(jsink)) >= mass_merger_vel_check * M_sun / (scale_d * scale_l ** ndim)) then
                    ...
                    if (merge_flag .eqv..true.) then
                        ...
                        if (stellar) then
                            do istellar = 1, nstellar
                                if (id_stellar(istellar) == idsink(jsink)) then
...
112 rewind(1)
    ...
    if ((create_sinks .or. accretion_scheme /= 'none') .and. (.not. hydro)) then
    ...
    if (create_sinks .or. (accretion_scheme /= 'none')) then
