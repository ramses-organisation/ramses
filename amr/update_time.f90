...
subroutine output_timer(write_file, filename)
    ...
    if (ncpu > 1) then
        ...
        if (id_is_one) then
            if (maxval(all_ntimer) /= minval(all_ntimer)) then
                write(ilun, *)
                write(ilun, *) '--------------------------------------------------------------------'
            ...
            gprint_timer = .false.
            do icpu = 1, ncpu
                if (all_ntimer(icpu) /= ntimer) then
...
subroutine update_time(ilevel)
...
#ifndef WITHOUTMPI
    if (myid == 1) then
        if (ttstart == 0.0) ttstart = MPI_WTIME()
    end if
#endif
    ...
    ! -------------------------------------------------------------
    if (nstep_coarse /= nstep_coarse_old) then
...
SUBROUTINE getProperTime(tau, tproper)
    ...
    real(dp) :: tau, tproper
    integer :: i
    if (.not. cosmo .or. tau == 0d0) then ! this might happen quite often
        tproper = tau
        return
    end if
    i = 1
