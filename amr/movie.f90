...
subroutine output_frame()
    ...
    if ((ndim == 2) .and. (trim(proj_axis) /= 'z')) proj_axis = 'z'
    ...
    do proj_ind = 1, LEN(trim(proj_axis))
        ...
        if ((ndim == 2) .and. (shader_frame(proj_ind) == 'cube')) shader_frame(proj_ind) = 'square'

        ! Some booleans for speedup
        is_cube = .false.
        if (shader_frame(proj_ind) == 'cube')      is_cube = .true.
        is_sphere = .false.
        if (shader_frame(proj_ind) == 'sphere')    is_sphere = .true.
        is_square = .false.
        if (shader_frame(proj_ind) == 'square')    is_square = .true.
        is_mean_mass = .false.
        if (method_frame(proj_ind) == 'mean_mass') is_mean_mass = .true.
        is_mean_dens = .false.
        if (method_frame(proj_ind) == 'mean_dens') is_mean_dens = .true.
        is_mean_vol = .false.
        if (method_frame(proj_ind) == 'mean_vol')  is_mean_vol = .true.
        is_mean_vol = .false.
        if (method_frame(proj_ind) == 'sum')       is_sum = .true.
        is_mean = .false.
        if (method_frame(proj_ind)(1:4) == 'mean') is_mean = .true.
        is_min = .false.
        if (method_frame(proj_ind) == 'min')       is_min = .true.
        is_max = .false.
        if (method_frame(proj_ind) == 'max')       is_max = .true.

        if (proj_axis(proj_ind:proj_ind) == 'x') then
            proj_ax = 1
        else if (proj_axis(proj_ind:proj_ind) == 'y') then
            proj_ax = 2
        else
            proj_ax = 3
        end if
        ...
        else if (levelmax_frame > nlevelmax) then
            nlevelmax_frame = nlevelmax
        else
        ...
        if (xcentre_frame(proj_ind * 4 - 3) == 0d0) xcentre_frame(proj_ind * 4 - 3) = boxlen / 2d0
        if (ycentre_frame(proj_ind * 4 - 3) == 0d0) ycentre_frame(proj_ind * 4 - 3) = boxlen / 2d0
        if (zcentre_frame(proj_ind * 4 - 3) == 0d0) zcentre_frame(proj_ind * 4 - 3) = boxlen / 2d0
        if (cosmo) then
            timer = aexp
        else
            timer = t
        ...
        if (deltax_frame(proj_ind * 2 - 1) == 0d0 .and. deltay_frame(proj_ind * 2 - 1) > 0d0) then
            deltax_frame(proj_ind * 2 - 1) = deltay_frame(proj_ind * 2 - 1) * float(nw_frame) / float(nh_frame)
        end if
        if (deltay_frame(proj_ind * 2 - 1) == 0d0 .and. deltax_frame(proj_ind * 2 - 1) > 0d0) then
        ...
        if (dist_camera(proj_ind) <= 0D0) dist_camera(proj_ind) = boxlen

        ! Camera properties
        if (cosmo) then
            if (tend_theta_camera(proj_ind) <= 0d0) tend_theta_camera(proj_ind) = aendmov
            if (tend_phi_camera(proj_ind) <= 0d0) tend_phi_camera(proj_ind) = aendmov
            ...
            if (tend_theta_camera(proj_ind) <= 0d0) tend_theta_camera(proj_ind) = tendmov
            if (tend_phi_camera(proj_ind) <= 0d0) tend_phi_camera(proj_ind) = tendmov
        ...
        if ((focal_camera(proj_ind) <= 0D0) .or. (focal_camera(proj_ind) > dist_camera(proj_ind))) focal_camera(proj_ind) = dist_cam
        ...
        ! Frame boundaries
        if (proj_ax == 1) then ! x-projection
        ...
        elseif (proj_ax == 2) then ! y-projection
                            ...
                            if ((ok(i)) .and. (ivar_frame(proj_ind) > 0) .and. (ivar_frame(proj_ind) <= nvar)) then
...
#endif
                                        ok(i) = ok(i) .and. (uvar >= varmin_frame(proj_ind))
                                        ok(i) = ok(i) .and. (uvar <= varmax_frame(proj_ind))
                                    end if
                                end do
                                ...
                                do i = 1, ngrid
                                    if (ok(i)) then
                                        ...
                                        pers_corr = 1.0
                                        if (proj_ax == 1) then ! x-projection
                                            if (dist_cam - xx(i, 1) < 0d0) cycle
                                            if (perspective_camera(proj_ind)) then
                                                alpha  = atan(xx(i, 2) / (dist_cam - xx(i, 1)))
                                                beta   = atan(xx(i, 3) / (dist_cam - xx(i, 1)))
                                                if (abs(alpha) / 2d0 > fov_camera) cycle
                                                if (abs(beta) / 2d0 > fov_camera) cycle
                                        ...
                                        zleft   = zcentre + minz
                                        zright  = zcentre + maxz
                                        if (    xright < xleft_frame .or. xleft >= xright_frame .or.&
                                            & yright < yleft_frame .or. yleft >= yright_frame .or.&
                                            & zright < zleft_frame .or. zleft >= zright_frame) cycle
#else
                                        xx(i, 1) = xx(i, 1) - xcen
                                        xx(i, 2) = xx(i, 2) - ycen
                                        ! Rotating
                                        ...
                                        if (    xright < xleft_frame .or. xleft >= xright_frame .or.&
                                            & yright < yleft_frame .or. yleft >= yright_frame) cycle
#endif
                                        ...
                                        do ii = imin, imax
                                            ...
                                            do jj = jmin, jmax
                                                ...
                                                if (abs(xxcentre - xleft) < 1d-2 * dx_frame) xpc = xpc - 1e-2 * dx_frame
                                                if (abs(yycentre - yleft) < 1d-2 * dx_frame) ypc = ypc - 1e-2 * dx_frame
                                                if (abs(xxcentre - xright) < 1d-2 * dx_frame) xpc = xpc - 1e-2 * dx_frame
                                                if (abs(yycentre - yright) < 1d-2 * dx_frame) ypc = ypc - 1e-2 * dx_frame
#if NDIM > 2
                                                if (is_cube) then
                                                    cube_face = .false.
                                                    if (sqrt(xpc ** 2 + ypc ** 2) > dx_proj * sqrt(3.0)) goto 666
                                                    ...
                                                    do iline = 1, 6
                                                        ...
                                                        if (d1 == - sign(d1, d2)) cube_face = .true.
                                                        ...
                                                        if (d3 == sign(d3, d4)) cube_face = .false.
                                                        if (cube_face) exit
                                                    end do
666                                                 continue
                                                end if
                                                ...
                                                if ((is_cube .and. (cube_face))                           &
                                                    .or. (is_sphere .and. sqrt(xpc ** 2 + ypc ** 2) <= dx_proj)  &
                                                    .or. (is_square .and. (abs(xpc) < dx_proj)             &
                                                    .and. (abs(ypc) < dx_proj)))                       then
                                                ! Intersection volume
                                                ...
                                                do kk = 1, n_movie_vars
                                                    ...
                                                    if (movie_vars(kk) == i_mv_temp) then
                                                        ok_frame = .true.
                                                        e = 0.0d0
#ifdef SOLVERmhd
                                                        do idim = 1, 3
                                                            ...
                                                            ! Density map
                                                        else if (movie_vars(kk) == i_mv_dens) then
                                                            ok_frame = .true.
                                                            uvar = uold(ind_cell(i), 1)

                                                            ! Pressure map
                                                        else if (movie_vars(kk) == i_mv_p) then
                                                            ok_frame = .true.
                                                            uvar = uold(ind_cell(i), ndim + 2)

                                                            ! Speed map
                                                        else if (movie_vars(kk) == i_mv_speed) then
                                                            ok_frame = .true.
                                                            uvar = 0.0d0
                                                            do idim = 1, ndim
                                                        ...
                                                        if (movie_vars(kk) == i_mv_xhi) then
                                                            ok_frame = .true.
                                                            if (ixhi /= 0) then
                ...
                do j = 1, npartmax
                    ...
                    if (dist_cam - zpf < 0) cycle

                    ! Check if particle is in the movie box
                    if (    xpf < xleft_frame .or. xpf >= xright_frame .or.&
                        & ypf < yleft_frame .or. ypf >= yright_frame .or.&
                        & zpf < zleft_frame .or. zpf >= zright_frame) cycle
#else
                    xpf  = xp(j, 1) - xcen
                    ypf  = xp(j, 2) - ycen
                    ...
                    ypf  = ypf + ycen
                    if (    xpf < xleft_frame .or. xpf >= xright_frame .or.&
                        & ypf < yleft_frame .or. ypf >= yright_frame) cycle
#endif
                    ...
                    do kk = 1, n_movie_vars
                        if (movie_vars(kk) == i_mv_dm .and. is_DM(typep(j))) then
                            ! DM particles
                        ...
                        else if (star .and. movie_vars(kk) == i_mv_stars .and. is_star(typep(j))) then
                            ! Star particles
                            data_frame(ii, jj, kk) = data_frame(ii, jj, kk) + mp(j)
                        else if (star .and. movie_vars(kk) == i_mv_lum .and. is_star(typep(j))) then
...
#endif
                if (myid == 1) then
                    if (method_frame(proj_ind)(1:3) /= 'sum') then
                        ! Convert into mass weighted
                        do kk = 1, n_movie_vars
                            if (movie_vars(kk) == i_mv_dm) cycle
                            if (movie_vars(kk) == i_mv_stars) cycle
                            if (movie_vars(kk) == i_mv_lum) cycle
                            do ii = 1, nw_frame
                                do jj = 1, nh_frame
                                    if ((is_mean) .and. (weights(ii, jj) > 0d0)) then
                                        data_frame(ii, jj, kk) = data_frame(ii, jj, kk) / weights(ii, jj)
                                    end if
                                    if (method_frame(proj_ind)(1:3) == 'min'.and. data_frame(ii, jj, kk) >= 1e-3 * huge(0.0)) then
                                        data_frame(ii, jj, kk) = 0.0
                                    end if
                                    if (method_frame(proj_ind)(1:3) == 'max'.and. data_frame(ii, jj, kk) <= - 1e-3 * huge(0.0)) then
                                        data_frame(ii, jj, kk) = 0.0
                                    end if
                                end do ! loop over jj
                            end do ! loop over ii
                ...
                ! Update counter
                if (proj_ind == len(trim(proj_axis))) then
                    ! Increase counter and skip frames if timestep is large
                    imov = imov + 1
                    do while ((amovout(imov) < aexp .or. tmovout(imov) < t) .and. (imov < imovout))
                        imov = imov + 1
                    end do
                end if
        ...
        subroutine set_movie_vars()
            ...
            n_movie_vars = 0
            do
                if (TRIM(movie_vars_txt(n_movie_vars + 1)) == '') exit
                n_movie_vars = n_movie_vars + 1
            end do

            do kk = 1, n_movie_vars
                if (movie_vars_txt(kk) == 'dm') then
                    if (i_mv_dm == - 1) i_mv_dm = kk
                    movie_vars(kk) = i_mv_dm

                else if (movie_vars_txt(kk) == 'stars') then
                    if (i_mv_stars == - 1) i_mv_stars = kk
                    movie_vars(kk) = i_mv_stars

                else if (movie_vars_txt(kk) == 'lum') then
                    if (i_mv_lum == - 1) i_mv_lum = kk
                    movie_vars(kk) = i_mv_lum

                else if (movie_vars_txt(kk) == 'temp') then
                    if (i_mv_temp == - 1) i_mv_temp = kk
                    movie_vars(kk) = i_mv_temp

                else if (movie_vars_txt(kk) == 'dens') then
                    if (i_mv_dens == - 1) i_mv_dens = kk
                    movie_vars(kk) = i_mv_dens

                else if (movie_vars_txt(kk) == 'P') then
                    if (i_mv_dens == - 1) i_mv_p = kk
                    movie_vars(kk) = i_mv_p

                else if (movie_vars_txt(kk) == '|v|') then
                    if (i_mv_dens == - 1) i_mv_speed = kk
                    movie_vars(kk) = i_mv_speed

                else if (movie_vars_txt(kk) == 'vx') then
                    if (i_mv_vx == - 1) i_mv_vx = kk
                    movie_vars(kk) = i_mv_vx

                else if (movie_vars_txt(kk) == 'vy') then
                    if (i_mv_vy == - 1) i_mv_vy = kk
                    movie_vars(kk) = i_mv_vy

                else if (movie_vars_txt(kk) == 'vz') then
                    if (i_mv_vz == - 1) i_mv_vz = kk
                    movie_vars(kk) = i_mv_vz

                else if (movie_vars_txt(kk) == 'Z') then
                    if (i_mv_metallicity == - 1) i_mv_metallicity = kk
                    movie_vars(kk) = i_mv_metallicity

                else if (movie_vars_txt(kk) == 'pmag') then
                    if (i_mv_pmag == - 1) i_mv_pmag = kk
                    movie_vars(kk) = i_mv_pmag

                else if (movie_vars_txt(kk) == 'xH2') then
                    if (i_mv_xh2 == - 1) i_mv_xh2 = kk
                    movie_vars(kk) = i_mv_xh2

                else if (movie_vars_txt(kk) == 'xHI') then
                    if (i_mv_xhi == - 1) i_mv_xhi = kk
                    movie_vars(kk) = i_mv_xhi

                else if (movie_vars_txt(kk) == 'xHII') then
                    if (i_mv_xhii == - 1) i_mv_xhii = kk
                    movie_vars(kk) = i_mv_xhii

                else if (movie_vars_txt(kk) == 'xHeII') then
                    if (i_mv_xheii == - 1) i_mv_xheii = kk
                    movie_vars(kk) = i_mv_xheii

                else if (movie_vars_txt(kk) == 'xHeIII') then
                    if (i_mv_xheiii == - 1) i_mv_xheiii = kk
                    movie_vars(kk) = i_mv_xheiii

                else if (index(movie_vars_txt(kk), 'var') == 1) then
                    if (i_mv_var == - 1) i_mv_var = kk
                    movie_vars(kk) = i_mv_var
                    ! Find which numbered variable to show
                    read( movie_vars_txt(kk)(4:5), '(i1)' ) ivar
                    movie_var_number(kk) = ivar

                else if (index(movie_vars_txt(kk), 'Fp') == 1) then
                    if (i_mv_fp == - 1) i_mv_fp = kk
