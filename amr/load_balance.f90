        ...
        do iy = 0, ny - 1
            do ix = 0, nx - 1
                ...
                if (ordering /= 'bisection') then
                    ...
                    cpu_map2(ind) = ncpu ! default value
                    do idom = 1, ndomain
                        if ( order_max(1) >= bound_key2(idom - 1) .and. &
                            & order_max(1) < bound_key2(idom)) then
        ...
        do ind = 1, twotondim
            ...
            if (ordering /= 'bisection') then
                ...
                do i = 1, ngrid
                    ...
                    do idom = 1, ndomain
                        if ( order_max(i) >= bound_key2(idom - 1) .and. &
                            & order_max(i) < bound_key2(idom)) then
...
subroutine cmp_cpumap(x, c, nn)
    ...
    if (ordering /= 'bisection') then
        ...
        do i = 1, nn
            ...
            do idom = 1, ndomain
                if (    order(i) >= bound_key(idom - 1) .and. &
                    & order(i) < bound_key(idom  )) then
                c(i) = idom
            end if
        end do
    end do
...
#if NDIM > 2
            if (zz > zc) order(i) = order(i) + 2d0 * acos(- 1d0)
#endif
        end do
    end if
#endif
