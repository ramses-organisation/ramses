...
! dbg ONLY !!!
!  write(fname1,100)ncount
!100 format('bsort',i4.4)
!  open(33,file=trim(fname1),form='formatted',status='unknown')
!!$  ngrid2=0
!!$  do ilevel=1,nlevelmax
...
!!$        if(ncache>0)then
!!$           igrid=istart
!!$           write(33,300) ilevel,ibound
!!$300 format('********   ILEV:',i3,'IBND',i3)
!!$           do i=1,ncache
!!$              write(33,200)ngrid2+i,i,igrid,xg(igrid,1:ndim)
!!$200 format(3i5.4,f6.3)
!!$              igrid=next(igrid)
!!$           end do
...
! dbg ONLY !!!
!                 write(33,200)ngrid2+i,i,igrid,xg(igrid,1:ndim),x(i,1:ndim),real(hilbert_keys_this_llist(i),kind=kind(0.0))
!200 format(3i5.4,6f9.3,f12.2)
...
!
! now apply this index mapping to all the global arrays
!
  ngrid2=0
  do igrid=1,igridmax
     flag2(igrid)=0
  ...
  do ilevel=1,nlevelmax
     do ibound=1,nboundary+ncpu
        ...
        igrid=istart
        do while(igrid/=0)
           hilbert_key(cpu_map2(igrid))=real(xg(igrid,idim),kind=qdp)
           igrid=next(igrid)
        enddo
     end do
  end do
  ...
  do ilevel=1,nlevelmax
     do ibound=1,nboundary+ncpu
        ...
        igrid=istart
        do while(igrid/=0)
           flag2(cpu_map2(igrid))=cpu_map(iskip2+igrid)
           igrid=next(igrid)
        end do
     end do
  end do
  ...
  do ilevel=1,nlevelmax
     do ibound=1,nboundary+ncpu
        ...
        igrid=istart
        do while(igrid/=0)
           flag2(cpu_map2(igrid))=flag1(iskip2+igrid)
           igrid=next(igrid)
        end do
     end do
  end do
  ...
  do ilevel=1,nlevelmax
     do ibound=1,nboundary+ncpu
        ...
        igrid=istart
        do while(igrid/=0)
           hilbert_key(cpu_map2(igrid))=real(uold(iskip2+igrid,ivar),kind=qdp)
           igrid=next(igrid)
        end do
     end do
  end do
  ...
  do ilevel=1,nlevelmax
     do ibound=1,nboundary+ncpu
        ...
        igrid=istart
        do while(igrid/=0)
           hilbert_key(cpu_map2(igrid))=real(f(iskip2+igrid,idim),kind=qdp)
           igrid=next(igrid)
        end do
     end do
  end do
...
! dbg ONLY !!!
!!$  write(fname1,101)ncount
!!$101 format('asort',i4.4)
!!$  open(33,file=trim(fname1),form='formatted',status='unknown')
!!$  ngrid2=0
!!$  do ilevel=1,nlevelmax
!!$     do ibound=1,nboundary+ncpu
...
!!$           write(33,300) ilevel,ibound
!!$           do i=1,ncache
!!$              write(33,201)ngrid2+i,i,igrid,xg(igrid,1:ndim)
!!$201 format(3i5.4,3f9.3)
!!$              igrid=next(igrid)
!!$           end do
!!$           ngrid2=ngrid2+ncache
!!$        end if
