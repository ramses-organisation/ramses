...
subroutine godunov_fine(ilevel)
  ...
  !--------------------------------------------------------------------------
  integer::i,ivar,igrid,jgrid,ncache,ngrid,ncache2
...
!$OMP PARALLEL DO DEFAULT(SHARED) PRIVATE(i,ngrid,igrid,ind_grid)
...
subroutine godfine1(ind_grid,ncache,ilevel)
  ...
  real(dp)::dx,dy,dz,scale,oneontwotondim,dtloc
...
!     call interpol_hydro(u1,g1,u2,g2,nbuffer)
  ...
  !-----------------------------------------------
  call unsplit(uloc,gloc,flux,tmp,dx,dy,dz,dtloc,ncache)
