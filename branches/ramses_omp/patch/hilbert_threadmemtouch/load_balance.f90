...
subroutine cmp_ordering(x,order,nn)
  ...
  if(ordering=='hilbert')then
     ...
     call map_pos2hilbert_key(x,order,nn,nvector)
  end if
...
subroutine defrag
  ...
  integer::max_ncache,nx_loc
  real(dp)::scale
  real(kind=dp),allocatable,dimension(:,:)::x
  real(qdp),allocatable,dimension(:)::hilbert_keys_this_llist
...
! dbg ONLY !!!
  ...
  integer, allocatable, dimension(:)    ::buf_int1d
  integer, allocatable, dimension(:,:)  ::buf_int2d
  real(dp),allocatable, dimension(:,:)  ::buf_dp2d
  real(dp),allocatable, dimension(:,:,:)::buf_dp3d
  ...
  nx_loc=icoarse_max-icoarse_min+1
  scale=boxlen/dble(nx_loc)
! find maximum length of the linked list blocks
  max_ncache=0
  do ilevel=1,nlevelmax
     do ibound=1,nboundary+ncpu
        ...
        max_ncache=max(max_ncache,ncache)
     enddo
  enddo
! get mem for that length
  allocate(x(max_ncache,ndim))
  allocate(hilbert_keys_this_llist(max_ncache))
  allocate(isort(max_ncache))

! get a hilbert key for each grid in those lists, sort according to those keys 
! within each list and store a corresponding global index mapping so that it can be
! applied to each global array
  ngrid2=0
  igridmax=0
  do ilevel=1,nlevelmax
     do ibound=1,nboundary+ncpu
        if(ibound<=ncpu)then
           ...
           if(ncache>0)then
              igrid=istart
! gather positions of all grids in this linked list
              do i=1,ncache
                 do idim=1,ndim
                    x(i,idim)=xg(igrid,idim)*scale
                 enddo
                 igridmax=max(igridmax,igrid)
                 igrid=next(igrid)
              end do
! assign hilbert keys to those positions and sort them
              call map_pos2hilbert_key(x,hilbert_keys_this_llist,ncache,max_ncache)
              call quick_sort(hilbert_keys_this_llist,isort,ncache)
! determine the future index of each grid after defragmentation based on the sorted
! list of hilbert keys
              igrid=istart
              do i=1,ncache
                 cpu_map2(igrid) = ngrid2+isort(i)
                 igrid=next(igrid)
              end do
              
              ngrid2=ngrid2+ncache
           end if
        else ! boundary grid, don't sort by hilbert key, just in order of access
           ncache=numbb(ibound-ncpu,ilevel)
           istart=headb(ibound-ncpu,ilevel)
           if(ncache>0)then
              igrid=istart
  ...
  deallocate(x)
  deallocate(hilbert_keys_this_llist)
  deallocate(isort)
...
!------------------------------------------
  ...
  deallocate(father) ! free memory
  allocate(father(1:ngridmax)) ! alloc again and then touch in omp loop below

  ngrid2=0
!$OMP PARALLEL DEFAULT(NONE) SHARED(numbl,numbb,flag2,father) PRIVATE(ilevel,ibound,ncache,igrid) FIRSTPRIVATE(nlevelmax,nboundary,ncpu,ngrid2)
  do ilevel=1,nlevelmax
     do ibound=1,nboundary+ncpu
        if(ibound<=ncpu)then
           ncache=numbl(ibound,ilevel)
...
!$OMP DO SCHEDULE(STATIC)
           do igrid=ngrid2+1,ngrid2+ncache
...
!------------------------------------------
  allocate(buf_int2d(1:ngridmax,1:twondim))
  buf_int2d(:,:)=-1000 ! just for dbg, not really needed
        ...
        do ibound=1,nboundary+ncpu
           ...
           if(ncache>0)then
              ...
              do i=1,ncache
                 ...
                 buf_int2d(cpu_map2(igrid),ind)=icell2
                 igrid=next(igrid)
              end do
              ngrid2=ngrid2+ncache
           end if
  ...
  deallocate(nbor) ! free memory
  allocate(nbor(1:ngridmax,1:twondim)) ! alloc again and then touch in omp loop below

!$OMP PARALLEL DEFAULT(NONE) SHARED(numbl,numbb,buf_int2d,nbor) PRIVATE(ind,ilevel,ibound,ncache,igrid,ngrid2) FIRSTPRIVATE(nlevelmax,nboundary,ncpu)
  do ind=1,twondim
     ngrid2=0
     do ilevel=1,nlevelmax
        do ibound=1,nboundary+ncpu
...
!$OMP DO SCHEDULE(STATIC)
              do igrid=ngrid2+1,ngrid2+ncache
                 nbor(igrid,ind)=buf_int2d(igrid,ind)
              enddo
!$OMP END DO
              ngrid2=ngrid2+ncache
           end if
...
!$OMP END PARALLEL  
  ...
  deallocate(buf_int2d)

!------------------------------------------
  allocate(buf_dp2d(1:ngridmax,1:ndim))
  buf_dp2d(:,:)=-1d20 ! just for dbg, not really needed
  ...
  do idim=1,ndim
     do ilevel=1,nlevelmax
        do ibound=1,nboundary+ncpu
           ...
           igrid=istart
           do while(igrid/=0)
              buf_dp2d(cpu_map2(igrid),idim)=xg(igrid,idim)
              igrid=next(igrid)
           end do
        end do
     end do
  ...
  deallocate(xg) ! free memory
  allocate(xg(1:ngridmax,1:ndim)) ! alloc again and then touch in omp loop below
!$OMP PARALLEL DEFAULT(NONE) SHARED(numbl,numbb,buf_dp2d,xg) PRIVATE(ind,ilevel,ibound,ncache,igrid,ngrid2) FIRSTPRIVATE(nlevelmax,nboundary,ncpu)
  do idim=1,ndim
     ngrid2=0
     do ilevel=1,nlevelmax
        do ibound=1,nboundary+ncpu
...
!$OMP DO SCHEDULE(STATIC)
              do igrid=ngrid2+1,ngrid2+ncache
                 xg(igrid,idim)=buf_dp2d(igrid,idim)
              enddo
!$OMP END DO
              ngrid2=ngrid2+ncache
           end if
...
!------------------------------------------
  ...
  do ilevel=1,nlevelmax
     do ibound=1,nboundary+ncpu
        ...
        igrid=istart
        do while(igrid/=0)
           flag2(cpu_map2(igrid))=headp(igrid)
           igrid=next(igrid)
        end do
     end do
  end do

  deallocate(headp) ! free memory
  allocate(headp(1:ngridmax)) ! alloc again and then touch in omp loop below

  ngrid2=0
!$OMP PARALLEL DEFAULT(NONE) SHARED(numbl,numbb,headp,flag2) PRIVATE(ilevel,ibound,ncache,igrid) FIRSTPRIVATE(nlevelmax,nboundary,ncpu,ngrid2)
  do ilevel=1,nlevelmax
     do ibound=1,nboundary+ncpu
        if(ibound<=ncpu)then
           ncache=numbl(ibound,ilevel)
...
!------------------------------------------
  ...
  do ilevel=1,nlevelmax
     do ibound=1,nboundary+ncpu
        ...
        igrid=istart
        do while(igrid/=0)
           flag2(cpu_map2(igrid))=tailp(igrid)
           igrid=next(igrid)
        end do
     end do
  end do

  deallocate(tailp) ! free memory
  allocate(tailp(1:ngridmax)) ! alloc again and then touch in omp loop below

  ngrid2=0
!$OMP PARALLEL DEFAULT(NONE) SHARED(numbl,numbb,flag2,tailp) PRIVATE(ilevel,ibound,ncache,igrid) FIRSTPRIVATE(nlevelmax,nboundary,ncpu,ngrid2)
  do ilevel=1,nlevelmax
     do ibound=1,nboundary+ncpu
        if(ibound<=ncpu)then
           ncache=numbl(ibound,ilevel)
...
!------------------------------------------
  ...
  do ilevel=1,nlevelmax
     do ibound=1,nboundary+ncpu
        ...
        igrid=istart
        do while(igrid/=0)
           flag2(cpu_map2(igrid))=numbp(igrid)
           igrid=next(igrid)
        end do
     end do
  end do

  deallocate(numbp) ! free memory
  allocate(numbp(1:ngridmax)) ! alloc again and then touch in omp loop below

  ngrid2=0
!$OMP PARALLEL DEFAULT(NONE) SHARED(numbl,numbb,flag2,numbp) PRIVATE(ilevel,ibound,ncache,igrid) FIRSTPRIVATE(nlevelmax,nboundary,ncpu,ngrid2)
  do ilevel=1,nlevelmax
     do ibound=1,nboundary+ncpu
        if(ibound<=ncpu)then
           ncache=numbl(ibound,ilevel)
...
!------------------------------------------
  ...
  allocate(buf_int2d(1:ngridmax,1:twotondim))
  buf_int2d(:,:)=-1000 ! just for dbg, not really needed
  allocate(buf_int1d(1:ncoarse))
  buf_int1d(:)=-1000 ! just for dbg, not really needed
     ...
     do ilevel=1,nlevelmax
        do ibound=1,nboundary+ncpu
           ...
           do while(igrid/=0)
              ...
              buf_int2d(cpu_map2(igrid),ind)=igrid2
              igrid=next(igrid)
           end do
        end do
     end do
  ...
  buf_int1d(1:ncoarse)=son(1:ncoarse)

  deallocate(son) ! free memory
  allocate(son(1:ncell)) ! alloc again and then touch in omp loop below

!$OMP PARALLEL DEFAULT(NONE) SHARED(numbl,numbb,buf_int2d,son) PRIVATE(ind,ilevel,ibound,ncache,igrid,iskip2,ngrid2) FIRSTPRIVATE(nlevelmax,nboundary,ncpu,ncoarse,ngridmax)
  do ind=1,twotondim
     iskip2=ncoarse+(ind-1)*ngridmax
     ngrid2=0
     do ilevel=1,nlevelmax
...
!$OMP DO SCHEDULE(STATIC)
              do igrid=ngrid2+1,ngrid2+ncache
                 son(iskip2+igrid)=buf_int2d(igrid,ind)
              enddo
!$OMP END DO
              ngrid2=ngrid2+ncache
           end if
...
!$OMP END PARALLEL  
  ...
  son(1:ncoarse)=buf_int1d(1:ncoarse) !maybe omp this and theo ther occurences???

  deallocate(buf_int1d)
...
!------------------------------------------
  ...
  do ind=1,twotondim
     ...
     do ilevel=1,nlevelmax
        do ibound=1,nboundary+ncpu
           ...
           igrid=istart
           do while(igrid/=0)
              buf_int2d(cpu_map2(igrid),ind)=cpu_map(iskip2+igrid)
              igrid=next(igrid)
           end do
        end do
     end do
  ...
  buf_int1d(1:ncoarse)=cpu_map(1:ncoarse)

  deallocate(cpu_map) ! free memory
  allocate(cpu_map(1:ncell)) ! alloc again and then touch in omp loop below

!$OMP PARALLEL DEFAULT(NONE) SHARED(numbl,numbb,buf_int2d,cpu_map) PRIVATE(ind,ilevel,ibound,ncache,igrid,iskip2,ngrid2) FIRSTPRIVATE(nlevelmax,nboundary,ncpu,ncoarse,ngridmax)
  do ind=1,twotondim
     iskip2=ncoarse+(ind-1)*ngridmax
     ngrid2=0
     do ilevel=1,nlevelmax
...
!$OMP DO SCHEDULE(STATIC)
              do igrid=ngrid2+1,ngrid2+ncache
                 cpu_map(iskip2+igrid)=buf_int2d(igrid,ind)
              enddo
!$OMP END DO
              ngrid2=ngrid2+ncache
           end if
...
!$OMP END PARALLEL  
  ...
  cpu_map(1:ncoarse)=buf_int1d(1:ncoarse)
...
!------------------------------------------
  ...
  do ilevel=1,nlevelmax
     do ibound=1,nboundary+ncpu
        ...
        igrid=istart
        do while(igrid/=0)
           buf_int2d(cpu_map2(igrid),ind)=flag1(iskip2+igrid)
           igrid=next(igrid)
        end do
     end do
  end do
  ...
  buf_int1d(1:ncoarse)=flag1(1:ncoarse)

  deallocate(flag1) ! free memory
  allocate(flag1(0:ncell)) ! alloc again and then touch in omp loop below

!$OMP PARALLEL DEFAULT(NONE) SHARED(numbl,numbb,buf_int2d,flag1) PRIVATE(ind,ilevel,ibound,ncache,igrid,iskip2,ngrid2) FIRSTPRIVATE(nlevelmax,nboundary,ncpu,ncoarse,ngridmax)
  do ind=1,twotondim
     iskip2=ncoarse+(ind-1)*ngridmax
     ngrid2=0
     do ilevel=1,nlevelmax
...
!$OMP DO SCHEDULE(STATIC)
              do igrid=ngrid2+1,ngrid2+ncache
                 flag1(iskip2+igrid)=buf_int2d(igrid,ind)
              enddo
!$OMP END DO
              ngrid2=ngrid2+ncache
           end if
...
!$OMP END PARALLEL  
  ...
  flag1(1:ncoarse)=buf_int1d(1:ncoarse)

  deallocate(buf_int1d)
  flag1(0)=0 ! really need this ???
...
#ifdef SOLVERmhd
  allocate(buf_dp3d(1:ngridmax,1:twotondim,1:nvar+3))
  buf_dp3d(:,:,:)=-1d20 ! just for dbg, not really needed

  allocate(buf_dp2d(1:ncoarse,1:nvar+3))
  buf_dp2d(:,:)=-1000 ! just for dbg, not really needed  

  do ivar=1,nvar+3

#else

  allocate(buf_dp3d(1:ngridmax,1:twotondim,1:nvar))
  buf_dp3d(:,:,:)=-1d20 ! just for dbg, not really needed

  allocate(buf_dp2d(1:ncoarse,1:nvar))
  buf_dp2d(:,:)=-1000 ! just for dbg, not really needed  
     ...
     do ind=1,twotondim
        ...
        do ilevel=1,nlevelmax
           do ibound=1,nboundary+ncpu
              ...
              igrid=istart
              do while(igrid/=0)
                 buf_dp3d(cpu_map2(igrid),ind,ivar)=uold(iskip2+igrid,ivar)
                 igrid=next(igrid)
              end do
           end do
        end do
     end do
     buf_dp2d(1:ncoarse,ivar)=uold(1:ncoarse,ivar)
  end do

  deallocate(uold) ! free memory
#ifdef SOLVERmhd
  allocate(uold(1:ncell,1:nvar+3)) ! alloc again and then touch in omp loop below
#else
  allocate(uold(1:ncell,1:nvar)) ! alloc again and then touch in omp loop below
#endif

!$OMP PARALLEL DEFAULT(NONE) SHARED(numbl,numbb,buf_dp3d,uold) PRIVATE(ind,ilevel,ibound,ncache,igrid,iskip2,ngrid2) FIRSTPRIVATE(nlevelmax,nboundary,ncpu,ncoarse,ngridmax)
#ifdef SOLVERmhd
  do ivar=1,nvar+3
#else
  do ivar=1,nvar
...
!$OMP DO SCHEDULE(STATIC)
                 do igrid=ngrid2+1,ngrid2+ncache
                    uold(iskip2+igrid,ivar)=buf_dp3d(igrid,ind,ivar)
                 enddo
!$OMP END DO
                 ngrid2=ngrid2+ncache
              end if
...
!$OMP END PARALLEL  
  ...
  do ivar=1,nvar
#endif
     uold(1:ncoarse,ivar)=buf_dp2d(1:ncoarse,ivar)
  enddo
...
!------------------------------------------
  ...
  allocate(buf_dp3d(1:ngridmax,1:twotondim,1:ndim))
  buf_dp3d(:,:,:)=-1d20 ! just for dbg, not really needed
  allocate(buf_dp2d(1:ncoarse,1:ndim))
  buf_dp3d(:,:,:)=-1d20 ! just for dbg, not really needed
     ...
     do ind=1,twotondim
        ...
        do ilevel=1,nlevelmax
           do ibound=1,nboundary+ncpu
              ...
              igrid=istart
              do while(igrid/=0)
                 buf_dp3d(cpu_map2(igrid),ind,idim)=f(iskip2+igrid,idim)
                 igrid=next(igrid)
              end do
           end do
        end do
     end do
     buf_dp2d(1:ncoarse,idim)=f(1:ncoarse,idim)
  end do

  deallocate(f) ! free memory
  allocate(f(1:ncell,1:3)) ! alloc again and then touch in omp loop below

!$OMP PARALLEL DEFAULT(NONE) SHARED(numbl,numbb,buf_dp3d,f) PRIVATE(ind,ilevel,ibound,ncache,igrid,iskip2,ngrid2) FIRSTPRIVATE(nlevelmax,nboundary,ncpu,ncoarse,ngridmax)
  do idim=1,ndim
     do ind=1,twotondim
        iskip2=ncoarse+(ind-1)*ngridmax
        ngrid2=0
...
!$OMP DO SCHEDULE(STATIC)
                 do igrid=ngrid2+1,ngrid2+ncache
                    f(iskip2+igrid,idim)=buf_dp3d(igrid,ind,idim)
                 enddo
!$OMP END DO
                 ngrid2=ngrid2+ncache
              end if
...
!$OMP END PARALLEL  
  ...
  do idim=1,ndim
     f(1:ncoarse,idim)=buf_dp2d(1:ncoarse,idim)
  enddo
...
!------------------------------------------
  ...
  do ind=1,twotondim
     ...
     do ilevel=1,nlevelmax
        do ibound=1,nboundary+ncpu
           ...
           igrid=istart
           do while(igrid/=0)
              buf_int2d(cpu_map2(igrid),ind)=rho(iskip2+igrid)
              igrid=next(igrid)
           end do
        end do
     end do
  ...
  buf_int1d(1:ncoarse)=rho(1:ncoarse)

  deallocate(rho) ! free memory
  allocate(rho(1:ncell)) ! alloc again and then touch in omp loop below

!$OMP PARALLEL DEFAULT(NONE) SHARED(numbl,numbb,buf_int2d,rho) PRIVATE(ind,ilevel,ibound,ncache,igrid,iskip2,ngrid2) FIRSTPRIVATE(nlevelmax,nboundary,ncpu,ncoarse,ngridmax)
  do ind=1,twotondim
     iskip2=ncoarse+(ind-1)*ngridmax
     ngrid2=0
     do ilevel=1,nlevelmax
...
!$OMP DO SCHEDULE(STATIC)
              do igrid=ngrid2+1,ngrid2+ncache
                 rho(iskip2+igrid)=buf_int2d(igrid,ind)
              enddo
!$OMP END DO
              ngrid2=ngrid2+ncache
           end if
...
!$OMP END PARALLEL  
  ...
  rho(1:ncoarse)=buf_int1d(1:ncoarse)
...
!------------------------------------------
  ...
  do ind=1,twotondim
     ...
     do ilevel=1,nlevelmax
        do ibound=1,nboundary+ncpu
           ...
           igrid=istart
           do while(igrid/=0)
              buf_int2d(cpu_map2(igrid),ind)=phi(iskip2+igrid)
              igrid=next(igrid)
           end do
        end do
     end do
  ...
  buf_int1d(1:ncoarse)=phi(1:ncoarse)

  deallocate(phi) ! free memory
  allocate(phi(1:ncell)) ! alloc again and then touch in omp loop below

!$OMP PARALLEL DEFAULT(NONE) SHARED(numbl,numbb,buf_int2d,phi) PRIVATE(ind,ilevel,ibound,ncache,igrid,iskip2,ngrid2) FIRSTPRIVATE(nlevelmax,nboundary,ncpu,ncoarse,ngridmax)
  do ind=1,twotondim
     iskip2=ncoarse+(ind-1)*ngridmax
     ngrid2=0
     do ilevel=1,nlevelmax
...
!$OMP DO SCHEDULE(STATIC)
              do igrid=ngrid2+1,ngrid2+ncache
                 phi(iskip2+igrid)=buf_int2d(igrid,ind)
              enddo
!$OMP END DO
              ngrid2=ngrid2+ncache
           end if
...
!$OMP END PARALLEL  
  ...
  phi(1:ncoarse)=buf_int1d(1:ncoarse)
...
!------------------------------------------
  ...
  if(allocated(rho_top))then
     ...
     do ind=1,twotondim
        ...
        do ilevel=1,nlevelmax
           do ibound=1,nboundary+ncpu
              ...
              igrid=istart
              do while(igrid/=0)
                 buf_int2d(cpu_map2(igrid),ind)=rho_top(iskip2+igrid)
                 igrid=next(igrid)
              end do
           end do
        end do
     ...
     buf_int1d(1:ncoarse)=rho_top(1:ncoarse)
     
     deallocate(rho_top) ! free memory
     allocate(rho_top(1:ncell)) ! alloc again and then touch in omp loop below

!$OMP PARALLEL DEFAULT(NONE) SHARED(numbl,numbb,buf_int2d,rho_top) PRIVATE(ind,ilevel,ibound,ncache,igrid,iskip2,ngrid2) FIRSTPRIVATE(nlevelmax,nboundary,ncpu,ncoarse,ngridmax)
     do ind=1,twotondim
        iskip2=ncoarse+(ind-1)*ngridmax
        ngrid2=0
        do ilevel=1,nlevelmax
...
!$OMP DO SCHEDULE(STATIC)
                 do igrid=ngrid2+1,ngrid2+ncache
                    rho_top(iskip2+igrid)=buf_int2d(igrid,ind)
                 enddo
!$OMP END DO
                 ngrid2=ngrid2+ncache
              end if
...
!$OMP END PARALLEL  
     ...
     rho_top(1:ncoarse)=buf_int1d(1:ncoarse)

     deallocate(buf_int1d)

  endif ! if(allocated(rho_top))
...
!#########################################################################
!#########################################################################
subroutine map_pos2hilbert_key(x,order,nn,nlen)

! This routine computes the keys along a Peano-Hilbert curve for a given set of
! positions. Note that this routine is called from cmp_ordering as well as from 
! defrag and the size of the argument arrays is different between the two.
  ...
  integer,intent(in) ::nn,nlen
  real(dp),dimension(1:nlen,1:ndim),intent(in)::x
  real(qdp),dimension(1:nlen),intent(out)::order

  integer,dimension(1:nlen)::ix,iy,iz
  integer::i,ncode,bit_length,nx_loc,temp,info
  ...
  if(bit_length==32) then
     write(*,*)'Error in map_pos2hilbert_key'
#ifndef WITHOUTMPI
     call MPI_ABORT(MPI_COMM_WORLD,1,info)
#else
     stop
...
! dbg ONLY !!!
!  print*,'II1:',nn,nlen,x(1:nn,1)
!     print*,'II2:',ix(1:nn),bscale
...
end subroutine map_pos2hilbert_key
