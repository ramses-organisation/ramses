...
do i = 1, npart
    ...
    if (btime == 0) then
        ...
        if (ix > 0 .and. ix <= np1 .and. iy > 0 .and. iy <= np2 .and. iz > 0 .and. iz <= np3) then
            imark(ix, iy, iz) = 1.0
        end if
            ...
            do j = 1, np2
                do k = 1, np3
                    if (imark(i, j, k) == 1.0) then
                        do ix = i - smt, i + smt
                            ix1 = ix
                            if (ix1 < 1) ix1 = ix1 + np1
                            if (ix1 > np1) ix1 = ix1 - np1
                            do iy = j - smt, j + smt
                                iy1 = iy
                                if (iy1 < 1) iy1 = iy1 + np2
                                if (iy1 > np2) iy1 = iy1 - np2
                                do iz = k - smt, k + smt
                                    iz1 = iz
                                    if (iz1 < 1) iz1 = iz1 + np3
                                    if (iz1 > np3) iz1 = iz1 - np3
                                    if (periodic) then
                                        r = (i - ix) ** 2 + (j - iy) ** 2 + (k - iz) ** 2
                                        r = r ** 0.5
                                        if (r <= smt .and. imark(ix1, iy1, iz1) == 0.0) then
                                        ...
                                        if (ix > 0 .and. ix <= np1 .and. iy > 0 .and. iy <= np2 .and. iz > 0 .and. iz <= np3) then
                                            r = (i - ix) ** 2 + (j - iy) ** 2 + (k - iz) ** 2
                                            r = r ** 0.5
                                            if (r <= smt .and. imark(ix, iy, iz) == 0.0) then
                                                imark(ix, iy, iz) = 2.0
                                            end if
                                        end if
                                    end if
            ...
            do j = 1, np2
                do k = 1, np3
                    if (imark(i, j, k) == 1.0) then
                        ...
                        do ix = i - smt, i + smt
                            ix1 = ix
                            if (ix1 <= 0) ix1 = ix1 + np1
                            if (ix1 > np1) ix1 = ix1 - np1
                            do iy = j - smt, j + smt
                                iy1 = iy
                                if (iy1 <= 0) iy1 = iy1 + np2
                                if (iy1 > np2) iy1 = iy1 - np2
                                do iz = k - smt, k + smt
                                    iz1 = iz
                                    if (iz1 <= 0) iz1 = iz1 + np3
                                    if (iz1 > np3) iz1 = iz1 - np3
                                    if (periodic) then
                                        r = (i - ix) ** 2 + (j - iy) ** 2 + (k - iz) ** 2
                                        r = r ** 0.5
