...
/* Feb 2020 - RS
Updates to make this compatible with the latest format of the RAMSES
part_ files. Make with -DMETALS if particles contain the metallicity
scalar.
 */
...
int ReadASCII(KD kd, FILE *fp)
{
  ...
  if (fgets(line, 200, fp) == NULL)
    f77error("Unexpected EOF in first line.");
  if (sscanf(line, "%d", &npart) != 1)
    f77error("Couldn't parse first line.");
...
/* Reading is done ZERO-OFFSET */
{
  ...
  if (fread(p, 1, size, f) != size)
    f77error("f77read(): Error reading data.");
...
int ReadRamses(KD kd, char *inputfile)
{
  ...
  int *temp_int;
  char *temp_byte; // RS Feb 2020 - Handles new arrays in part_ files
  int *temp_id;
  float pmax;
  FILE *fp, *fascii;
  char *tempi;
  ...
  fread(&dummy, sizeof(dummy), 1, fp);
  printf("\tnstar %d\n", nstar);
  printf("\tnpart - nstar %d\n", npart - nstar);
  // RS - Feb 2020 adding fields in updated RAMSES
  fread(&dummy, sizeof(dummy), 1, fp);
  ...
  fread(&dummy, sizeof(dummy), 1, fp);
  printf("mstar_tot %d, mstar_lost %d, nsink %d\n", mstar_tot,
         mstar_lost, nsink);
 ...
 */
  printf("nstar= %d \n", nstar);
  if (nstar > 0)
    printf("Found stars in Ramses files. \n");
  fclose(fp);
  ...
  for (icpu = 1; icpu <= ncpu; icpu++)
  {
    ...
    fread(&dummy, sizeof(dummy), 1, fp);
    fread(&dummy, sizeof(dummy), 1, fp);
    fread(&npart2, sizeof(npart2), 1, fp); // Tot parts in file
    fread(&dummy, sizeof(dummy), 1, fp);
    fclose(fp);
    npart = npart + npart2;
  }
  ...
  if (nstar > 0)
  {
    ...
    for (icpu = 1; icpu <= ncpu; icpu++)
    {
      if (icpu % 10 == 0)
      {
        printf("Working on cpu #%d\n", icpu);
      }
      // I think this loop just counts DM particles... RS
      sprintf(currfile, tempi, icpu);
      ...
      fread(&dummy, sizeof(dummy), 1, fp);
      //      printf("\tncpu %d, ndim %d, npart2 %d, nstar %d\n",
      //             ncpu, ndim, npart2, nstar_tot);
      ...
      //  RS - Feb 2020 new byte arrays in part output.
      temp_byte = (char *)malloc(npart2 * sizeof(char));
      ...
      //Skipping family - RS Feb 2020 - 1 byte long
      fread(&dummy, sizeof(dummy), 1, fp);
      fread(&temp_byte[0], sizeof(char), npart2, fp);
      fread(&dummy, sizeof(dummy), 1, fp);

      //Skipping tag - RS Feb 2020 - 1 byte long
      fread(&dummy, sizeof(dummy), 1, fp);
      fread(&temp_byte[0], sizeof(char), npart2, fp);
      fread(&dummy, sizeof(dummy), 1, fp);
      ...
      fread(&temp_dbl[0], sizeof(double), npart2, fp);
      fread(&dummy, sizeof(dummy), 1, fp);
      // Use age > 0 to find star particles
      for (j = 0; j <= npart2 - 1; j++)
      {
        if (temp_dbl[j] == 0. && temp_id[j] > 0)
          ncurr = ncurr + 1;
      }
    ...
    npart = ncurr; // Number of DM particle across all CPUs - RS
  }

  printf("Actual number of DM particles = %d \n", npart);
  printf("Corresponds to levelmin = %3.0f \n", log2(npart) / ndim);
  ...
  for (icpu = 1; icpu <= ncpu; icpu++)
  {
    ...
    {
      //Skipping identity
      fread(&dummy, sizeof(dummy), 1, fp); // RS - all these dummies... fortran buffer bytes?
      fread(&temp_id[0], sizeof(int), npart2, fp);
      fread(&dummy, sizeof(dummy), 1, fp);
...
#ifdef METALS
      // RS - Feb 2020 Need to add IF statements for Z, P
      //Reading metallicity
      fread(&dummy, sizeof(dummy), 1, fp);
      fread(&temp_dbl[0], sizeof(double), npart2, fp);
      fread(&dummy, sizeof(dummy), 1, fp);
