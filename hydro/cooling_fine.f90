...
subroutine coolfine1(ind_grid, ngrid, ilevel)
    ...
    if (.not. cosmo .and. haardt_madau .and. aexp_ini <= 1.)              &
        aexp_loc = aexp_ini
#endif
    ...
    do ind = 1, twotondim
        ...
        ! ==========================================
        if (barotropic_eos .and. (barotropic_eos_form /= 'legacy')) then
            do i = 1, nleaf
                ! analytic EOS
        ...
        if (neq_chem) then
            ...
            if (cooling .and. delayed_cooling .and. .not. cooling_ism) then
                ...
                do i = 1, nleaf
                    if (uold(ind_leaf(i), idelay) / uold(ind_leaf(i), 1) > 1d-3) &
                        cooling_on(i) = .false.
                end do
            end if
        ...
        if (rt_isIRtrap) then
            ...
            do i = 1, nleaf
                ...
                if (tau > 0d0) f_trap = min(max(exp(- 1d0 / tau), 0d0), 1d0)
