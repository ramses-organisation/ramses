subroutine init_hydro
    ...
    if (nrestart > 0) then
...
#ifdef RT
        if ((neq_chem .or. rt) .and. nvar2 < nvar) then ! OK to add ionization fraction vars
        ...
        if ((neq_chem .or. rt) .and. nvar2 > nvar) then ! Not OK to drop variables
            if (myid == 1) write(*, *) 'File hydro.tmp is not compatible'
                ...
                if (ncache > 0) then
                    ...
                    do ind = 1, twotondim
                        ...
                        do ivar = ndim + 3, ndim + 2 + nener
                            if (remap_pscalar(ivar - ndim - 2) >- 1) read(ilun) xx
                            do i = 1, ncache
                                if (remap_pscalar(ivar - ndim - 2) > 0) then
                                    uold(ind_grid(i) + iskip, remap_pscalar(ivar - ndim - 2)) = xx(i) / (gamma_rad(ivar - ndim - 2) - 1d0)
                                else if (remap_pscalar(ivar - ndim - 2) < 0) then
